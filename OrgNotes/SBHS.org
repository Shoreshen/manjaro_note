...
任取[[~/OneDrive/离散数学/Disc_Math.org:MK48][至多可数]]个 $[a,b]$ 上互不相交的开区间 $\{(a_i,b_i)\}$ 若满足 $\sum_{i=1}^n(b_i-a_i)<\delta$
...
由于是绝对连续函数，故对于 $\epsilon=1$ 存在 $\delta>0$ 对于任意[[~/OneDrive/离散数学/Disc_Math.org:MK48][至多可数]]个互不相交的开区间 $\{(a_i,b_i)\}$
...
则对于 $[a_i,b_i]$ 上的任意[[~/OneDrive/离散数学/Disc_Math.org::MK360][有理划分]] $T=\{[a_{ij},b_{ij}]\}$ 根据条件均满足 $Z_{T,[a_i,b_i]}(f)<1$

故根据[[~/OneDrive/离散数学/Disc_Math.org::MK339][定理2]]可得 $\sup(Z_{[a_i,b_i]}(f))\leq1$ 其中 $Z_{[a_i,b_i]}(f)=\left\{Z_{T,[a_i,b_i]}(f)\big|T\in\mathcal{T}_i\right\}$ 即 $\overunderset{b_i}{a_i}{V}(f)\leq1$
...
故对于任意 $\epsilon>0$ 存在 $\delta$ 任取[[~/OneDrive/离散数学/Disc_Math.org:MK48][至多可数]]个 $[a,x_0]$ 上互不相交的开区间 $\{(a_i,b_i)\}$
...
（1） [[~/OneDrive/大学物理/Note/DiffEq.org::MK20][导数]] $f'(x)$ [[MK190][几乎处处]]存在，且
...
（1） $g(x),h(x)$ 在 $[a,b]$ 上[[MK190][几乎处处]]存在[[~/OneDrive/大学物理/Note/DiffEq.org::MK20][导数]]
...
则对于任意 $\epsilon>0$ 存在[[~/OneDrive/离散数学/Disc_Math.org::MK42][有限]]个区间 $I_1,I_2,...,I_n}$ 使得
