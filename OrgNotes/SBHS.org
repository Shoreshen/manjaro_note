...
定义 $Y\subset X$ 且 $Y$ 是 $<X,\varphi>$ 上的外侧度可测集
...
（2） 对于任意 $C\subset B$ 且 $C$ 为[[~/OneDrive/数学分析/Chap11Note.org::MK10][开集]]，则 $C^{-1}=\left\{\vec{x}\big|f(\vec{x})\in C\right\}$ 仍为开集 
...
若 $f_i:E_i\to\mathbb{R}_e$ 为 $<E_i,\varphi>$ 上的[[MK134][可测函数]]，则 $f(x)$ 亦为可测函数
...
故根据[[MK176][引理]]可得 $f_i^{-1}((a,+\infty])$ 亦为 $<E,\varphi>$ 上的[[MK174][外测度可测集]]
...
（1） 若 $f(x)$ 为[[MK134][可测函数]]，则存在非负[[MK178][简单函数]]列 $\{\phi_i(x)\}$ [[~/OneDrive/数学分析/Chap10Note.org::MK2][收敛]]于 $f(x)$ 即 $\lim_{i\to\infty}\phi_i(x)=f(x)$

（2） 若满足（1）且 $f(x)$ 有界，则 $\{\phi_i(x)\}$ [[~/OneDrive/数学分析/Chap10Note.org::MK1][一致收敛]]与 $f(x)$

***** 证明

****** 构建函数

定义函数 $\phi_i(x)\begin{cases}i&f(x)>i\\\frac{j-1}{2^i}&\frac{j-1}{2^i}\leq f(x)<\frac{j}{2^i},j=1,2,...,2^i\end{cases}$
...
进一步定义分段函数 $f(x)=f_i(x),\ x\in E_i$ 其中 $f_i(x)=c_i,c_i\in\mathbb{R}_e$ 则 $f(x)$ 亦为[[MK134][可测函数]]
