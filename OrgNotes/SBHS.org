...
设 $T$ 是 $X$ 的非空子集的[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合定义-集合族]]

定义闭集为 $t^c:t\in T$ 定义闭集族 $T^c=\left\{t^c\big|t\in T\right\}$ 则 $<X,T>$ 是[[MK39][拓扑空间]]的充要条件是：
...
定义对于[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合定义-集合族]] $A$ 的[[~/OneDrive/离散数学/Disc_Math.org][集合论-计算-广义并]]为 $\bigcup A$ 同理定义广义交为 $\bigcap A$

定义 $F\subset T$ 以及 $F^c=\left\{f^c\big|f\in F\right\}$ 根据 $T^c$ 定义显然有 $F^c\subset T^c$

则根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-计算-性质-集合族运算与补]]可得 $\left(\bigcup F\right)^c=\bigcap\left\{f^c\big|f\in F\right\}=\bigcap F^c$
...
根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-计算-性质-集合族运算与交并]]有 $Y\cap\bigcup F=\bigcup\left\{f\cap Y\big|f\in F\right\}=\bigcup F_Y$
...
则当 $Y\in T\lor Y^c\in T$ 时有 $x\in T_Y\rightarrow x\in T$

**** 证明

若 $Y\in T$ 则根据定义对于任意 $y\in T_Y$ 存在 $x\in T$ 满足 $x\cap Y=y$ 根据[[MK39][开集公理]]（3）可得 $y\in T$ 

若 $Y^c\in T$ 则可用[[MK129][闭集定义]]证明
...
对于满足[[~/OneDrive/离散数学/Disc_Math.org][关系与函数-函数-定义-函数定义]]的函数 $f:X\to Y$ 

其 $G\subset Y$ 的原相定义为 $g(G)=\left\{x\big|f(x)\in G\land x\in X\right\}$

**** 性质<<MK135>>

定义 $Y_\lambda\subset Y$ 则有

（1） $\left\{x\big|f(x)\in\left(\bigcup_{\lambda\in\Lambda} Y_i\right)\land x\in X\right\}=\bigcup_{\lambda\in\Lambda}\left\{x\big|f(x)\in Y_i\land x\in X\right\}$ 

（2） $\left\{x\big|f(x)\in\left(\bigcap_{\lambda\in\Lambda} Y_i\right)\land x\in X\right\}=\bigcap_{\lambda\in\Lambda}\left\{x\big|f(x)\in Y_i\land x\in X\right\}$ 

（3） $\left\{x\big|f(x)\in Y^c\land x\in X\right\}=\left(\left\{x\big|f(x)\in Y\land x\in X\right\}\right)^c$

***** 证明

根据[[~/OneDrive/离散数学/Disc_Math.org][关系与函数-函数-相关定义-函数定义]] $\forall x\in X\rightarrow\exists y\left(y\in Y\land f(x)=y\right)$ 

定义 $g(G)=\left\{x\big|f(x)\in G\land x\in X\right\}$ 故有以下证明

****** （1）

则对于任意 $x\in g\left(\bigcup_{\lambda\in\Lambda} Y_i\right)$ 根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合的计算-运算]]可得 $f(x)\in\bigcup_{\lambda\in\Lambda} Y_i\leftrightarrow\exists \lambda^*\in\Lambda\rightarrow f(x)\in Y_{\lambda^*}$

故对于任意 $x\in g\left(\bigcup_{\lambda\in\Lambda} Y_i\right)\leftrightarrow\left(\exists \lambda^*\in\Lambda\rightarrow f(x)\in Y_{\lambda^*}\right)\leftrightarrow x\in g(Y_{\lambda^*})\leftrightarrow x\in\bigcup_{\lambda\in\Lambda} g(Y_\lambda)$

故根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合定义-ZFC公理化集合论-外延公理]]可得 $g\left(\bigcup_{i=1}^\infty X_i\right)=\bigcup_{i=1}^\infty g(X_i)$

****** （2）

则对于任意 $x\in g\left(\bigcap_{\lambda\in\Lambda} Y_i\right)$ 根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合的计算-运算]]可得 $f(x)\in\bigcap_{\lambda\in\Lambda} Y_i\leftrightarrow\forall\lambda\in\Lambda\rightarrow f(x)\in Y_{\lambda^*}$

故对于任意 $x\in g\left(\bigcap_{\lambda\in\Lambda} Y_i\right)\leftrightarrow\left(\forall \lambda\in\Lambda\rightarrow f(x)\in Y_{\lambda}\right)\leftrightarrow \forall\lambda\left(x\in g(Y_{\lambda})\right)\leftrightarrow x\in\bigcup_{\lambda\in\Lambda} g(Y_\lambda)$

故根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合定义-ZFC公理化集合论-外延公理]]可得 $g\left(\bigcap_{i=1}^\infty X_i\right)=\bigcap_{i=1}^\infty g(X_i)$

****** （3）

则对于任意 $x\in g\left(Y^c\right)$ 根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合的计算-运算]]可得 $f(x)\in Y^c\leftrightarrow f(x)\not\in Y$

故对于任意 $x\in g(Y^c)\leftrightarrow f(x)\not\in Y\leftrightarrow x\not\in g(Y)\leftrightarrow (g(Y))^c$

故根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合定义-ZFC公理化集合论-外延公理]]可得 $g(Y^c)=(g(Y))^c$

*** 连续函数<<MK115>>

函数 $f:X\to Y$ 其中 $X,Y$ [[MK39][拓扑空间]]
...
对于集合 $X$ 存在 $F\subset X$ 定义 $G=\left\{F\cap g\big|g\in\sigma(X)\right\}$ 则 $G$ 是 $F$ 上的[[MK50][ $\sigma$ 代数]]
...
由于 $\sigma(F)$ 是基于 $F$ 产生的 $\sigma$ 代数，故其本身应为 $F$

根据定义有 $X\in\sigma(X)$ 又由于 $F\subset X$ 则有 $F\cap X=F$ ，即满足 $F\in G$
...
根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合的计算-性质-分配律]]可得 $F\cap\left(\bigcup_{i=1}^\infty x_i\right)=\bigcup_{i=1}^\infty(x_i\cap F)=\bigcup_{i=1}^\infty f_i\in G$
...
（1）[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合的计算-性质-德摩根律]]

（2）[[~/OneDrive/离散数学/Disc_Math.org][命题逻辑-命题公式-逻辑等价-逻辑等价式举例]]-德摩根律

（3）[[~/OneDrive/离散数学/Disc_Math.org][命题逻辑-命题公式-逻辑等价-逻辑等价式举例]]-分配率
...
故根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合定义-ZFC公理化集合论-外延公理]] $g^{c_F}=x^{c_X}\cap F\in G$
...
若 $\Phi\subset\rho(X)$ [[MK85][生成]]了 $X$ 上的[[MK50][ $\sigma$ 代数]] $\sigma_\Phi(X)$ 则对于 $Y\subset X$ 定义集合 $\Psi=\left\{x\cap Y\big|x\in\beta\right\}$

此时由 $\Psi$ [[MK85][生成]]的 $Y$ 上的[[MK50][ $\sigma$ 代数]] $\sigma_\Psi(Y)=\left\{Y\cap x\big|x\in\sigma_\Phi(X)\right\}$

***** 证明

定义 $L=\left\{Y\cap x\big|x\in\sigma_\Phi(X)\right\}$ 根据[[~/OneDrive/离散数学/Disc_Math.org][集合论-集合的关系-子集-自反性]]证明以下结论，故有 $L=\sigma_{\Psi}(Y)$
...
（4） 存在加/乘法[[~/OneDrive/离散数学/Disc_Math.org][抽象代数-特殊元素-幺元]]： $\exists 0\in\mathbb{R}\forall a\in\mathbb{R}\left(0+a=a\right)\land\exists1\in\mathbb{R}\forall a\in\mathbb{R}\left(1\cdot a=a\right)$

（5） 存在加/乘法[[~/OneDrive/离散数学/Disc_Math.org][抽象代数-特殊元素-逆元]]： $\forall a\in\mathbb{R}\exists -a\in\mathbb{R}\left(-a+a=0\right)\land\forall a\in\mathbb{R}\exists a^{-1}\in\mathbb{R}\left(a^{-1}\cdot a=1\right)$
...
根据[[~/OneDrive/数学分析/Chap11Note.org][向量值函数-向量值函数连续]]，在不考虑边界点的情况下（只考虑 $B$ 的[[~/OneDrive/数学分析/Chap11Note.org][构造Euclid空间-点的定义-内点]]）
...
（2） 对于任意 $C\subset B$ 且 $C$ 为[[~/OneDrive/数学分析/Chap11Note.org][构造Euclid空间-集合定义-开集]]，则 $C^{-1}=\left\{\vec{x}\big|f(\vec{x})\in C\right\}$ 仍为开集 
...
若 $C^{-1}=\varnothing$ 则根据[[~/OneDrive/数学分析/Chap11Note.org][构造Euclid空间-集合定义-部分定理-定理8]]可得空集是开集，即开集 $C$ 的原相是开集

若 $C^{-1}\ne\varnothing$ 则对于任意 $\vec{x}_0\in C^{-1}$ 均有 $f(\vec{x}_0)\in C$ 由于 $C$ 是开集，故有 $\exists\epsilon>0\rightarrow O(f(\vec{x}_0),\epsilon)\subset C$
...
综上所述，对于任意 $\vec{x}_0\in C$ 均存在 $\delta>0\rightarrow O(\vec{x}_0,\delta)\subset C$ 即 $C$ 的所有点均为[[~/OneDrive/数学分析/Chap11Note.org][构造Euclid空间-点的定义-内点]]

故根据[[~/OneDrive/数学分析/Chap11Note.org][构造Euclid空间-集合定义-开集]]定义 $C$ 是开集。即（2）推出（1）

****** 必要

对于任意 $\vec{x}_0\in B$ 以及 $\epsilon>0$ 定义集合 $Y=\left\{f(\vec{x})\big|\left|f(\vec{x})-f(\vec{x}_0)\right|<\epsilon\right\}$ 根据领域定义 $Y=O(f(\vec{x}_0),\epsilon)$

根据[[~/OneDrive/数学分析/Chap11Note.org][构造Euclid空间-集合定义-部分定理-定理2]]可得领域为开集，即 $O(f(\vec{x}_0),\epsilon)$ 是开集
...
故根据[[~/OneDrive/数学分析/Chap11Note.org][构造Euclid空间-集合定义-开集]]定义有 $\eixsts\delta>0\rightarrow O(\vec{x},\delta)\subset\left\{\vec{x}\big|\left|f(\vec{x})-f(\vec{x}_0)\right|<\epsilon\right\}$
...
定义函数 $f:E\to\mathbb{R}\cup\{-\infty,+\infty\}$ 其中 $\{-\infty,+\infty\}$ 满足对于任意 $a\in\mathbb{R}$ 均有：
...
对于 $\{-\infty,+\infty\}$ 以及[[MK4][加法幺元]] $0$ 满足：
...
则定义 $f(x),x\in E$ 为广义实值函数

若 $f:E\to\mathbb{R}$ 则定义为有限函数

** 可测函数<<MK134>>

对于[[MK133][广义实值函数]] $f:E\to\mathbb{R}\cup\{-\infty,+\infty\},E\subset X$ 

定义 $E[f>a]=\left\{x\big|f(x)>a\land x\in E\right\}$ 以及 $\matchal{M}\subset\rho(X)$ 是 $X$ 上所有的[[MK65][可测]]集族

则若 $f$ 满足 $\forall a\in\mathbb{R}\rightarrow E[f>a]\in\mathcal{M}$ 则 $f$ 是可测函数

*** 等价定义

以下条件与[[MK134][可测函数]]定义等价：

（1）$\forall a\in\mathbb{R}\rightarrow E[f\leq a]\in\mathcal{M}$

（2）$\forall a\in\mathbb{R}\rightarrow E[f\geq a]\in\mathcal{M}$

（3）$\forall a\in\mathbb{R}\rightarrow E[f>a]\in\mathcal{M}$
...
由于 $f(x)>a=f(x)\in[a,+\infty]$ 定义集合 $\bigcap_{i=1}^\infty \left(a-\frac{1}{i},+\infty\right)$

则有 $\forall z\in\bigcap_{i=1}^\infty \left(a-\frac{1}{i},+\infty\right]\leftrightarrow\left(\forall i\in\mathbb{N}\rightarrow z\in\left(a-\frac{1}{i},+\infty\right]\right)\leftrightarrow z\in[a,\infty]$

故有 $E[f\geq a]=\left\{x\big|f(x)\in\bigcap_{i=1}^\infty\left(a-\frac{1}{n},+\infty\right]\land x\in E\right\}$

根据[[MK135][性质]]（2）有 $\left\{x\big|f(x)\in\bigcap_{i=1}^\infty\left(a-\frac{1}{n},+\infty\right]\land x\in E\right\}=\bigcap_{i=1}^\infty\left\{x\big|f(x)\in \left(a-\frac{1}{n},+\infty\right]\land x\in X\right\}$

故有 $E[f\geq a]=\bigcap_{i=1}^\infty E\left[f>a-\frac{1}{n}\right]$ 同理可证 $E[f>a]=\bigcap E\left[f\geq a+\frac{1}{n}\right]$

故根据[[MK50][定义]]中（3）可得互为充要条件

***** （3）

根据[[MK136][（2）]]中的结论利[[MK137][（1）]]中的证明方法可得
