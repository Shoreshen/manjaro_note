...
（3） 可数可加性： $\rho(\bigcup_{i=1}^\infty A_i)=\sum_{i=1}^\infty\rho(A_i)$ 注意此处 $i$ 为[[~/OneDrive/离散数学/Disc_Math.org::MK44][可数]]个，且当 $i\ne j$ 时 $A_i\cap A_j=\varnothing$
...
定义 $E[f>a]=\left\{x\big|f(x)\in(a,+\infty]\land x\in E\right\}$ 即 $(a,\infty]$ 的[[MK151][原相]]

定义 $\mathcal{M}\subset\rho(E)$ 是 $E$ 上所有的[[MK65][外测度可测集]]族
...
若存在[[~/OneDrive/离散数学/Disc_Math.org::MK48][之多可数]]个函数 $f_i(x), i\in I$ 定义 $\phi(x)=\inf\left\{f_i(x)\big|i\in I\right\},\psi(x)=\sup\left\{f_i(x)\big|i\in I\right\}$

则 $\phi(x),\psi(x)$ 仍为[[MK134][可测函数]]

**** 证明

由于 $\phi^{-1}((a,+\infty])=\left\{x\big|\phi(x)\in(a,+\infty]\right\}$ 且 $\phi(x)=\inf\left\{f_i(x)\big|i\in I\right\}$

即 $\forall x\in \phi^{-1}((a,+\infty])\rightarrow \inf\left\{f_i(x)\big|i\in I\right\}\in(a,+\infty]$ 
