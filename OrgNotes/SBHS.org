...
则对于 $x\in[a,b]$ 上[[MK190][几乎处处]]有 $$\lim_{h\to0}\frac{1}{h}\int_{[0,h]}\left|f(x+t)-f(x)\right|dt=0$$

*** 证明

仅证 $h>0$ 的情况 $h<0$ 类似可证，根据条件结合[[MK281][性质8]]

可得其充要条件为 $$\int_{[a,b]}\left\{\lim_{h\to0^+}\frac{1}{h}\int_{[0,h]}\left|f(x+t)-f(x)\right|dt\right\}dx=0$$

定义 $h(\vec{z})=\left|f(x+t)-f(x)\right|$ 其中 $\vec{z}=<x,t>$



则 $$\int_{[a,b]\times[0,b-a]}$$

由于 $f(x)$ 勒贝格可积，故 $f(x)$ 为[[MK134][可测函数]]，故对于任意 $a\in\mathbb{R}$ 有 $f^{-1}([a,+\infty))$ 是 $\mathbb{R}$ 上的[[MK81][可测集]]
...
则对于任意 $a\in\mathbb{R}$ 可得 $h^{-1}([a,\infty))=\left\{<x,t>\big|f(x+t)-f(x)\geq a\right\}$
