...
若 $f(x)$ 是闭[[MK13][区间]] $[a,b]$ 上的单调函数，则有

（1） $f(x)$ 在 $[a,b]$ 上几乎处处存在[[~/OneDrive/大学物理/Note/DiffEq.org::MK20][导数]]（可以是无穷）

（2） $f(x)$ 在 $[a,b]$ 上[[MK237][勒贝格可积]]
...
设 $g(x)=f(x)+x$ 若 $g(x)$ 可导，则有 $g'(x)=\lim_{x\to x_0}\frac{g(x)-f(x)}{x-x_0}=A\in\mathbb{R}_e$
...
则对于 $x_0\in E$ 根据[[MK336][推论2]]可得至少存在两个不相等的列导数，假设 $\mathcal{D}_{h_1}(x_0)>\mathcal{D}_{h_2}(x_0)$

根据[[MK43][有理数稠密性]]可得存在有理数 $p,q$ 满足 $\mathcal{D}_{h_1}(x_0)>p>q>\mathcal{D}_{h_2}(x_0)$

定义 $E_{p,q}=\left\{x|x\in E\land \mathcal{D}_{h_1}(x_0)>p>q>\mathcal{D}_{h_2}(x_0)\right\}$

则根据上述说法显然 $E\subset\bigcup_{<p,q>\in\mathbb{Q}\times\mathbb{Q}}E_{p,q}$

由于严格递增，易证列导数必为正，则有 $r>0$ 则对于 $E_r$ 根据[[MK337][引理]]有
