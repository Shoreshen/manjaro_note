...
若 $R\subset A_1\times...\times A_n$ 则称 $R$ 为集合 $A_1,...,A_n$ 上的 $n$ 元关系
...
$\varnothing\subset A\times B$ 即 $R=\varnothing$
...
若 $f_1,...,f_n:X\rightarrow Y,g:Y^n\rightarrow Z$ 则 $g\cdot (f_1,...,f_n)$ 满足对于任意 $\left<x_1,...,x_n\right>\in X^n$

均有 $g\cdot(f_1,...,f_n)=g(f_1(x_1),...,f_n(x_n))$ 亦为[[MK175][函数]]

****** 证明

由于 $f_i$ 均为[[MK175][函数]]，则对于任意 $x_i\in X$ 有且仅有一个 $f_i(x_i)\in Y$ 满足 $\left<x_i,f_i(x_i)\right>\in f_i$

即对于任意 $\left<x_1,...,x_n\right>\in X^n$ 有且仅有一组 $\left<f_1(x_1),...,f_n(x_n)\right>\in Y^n$ 满足 $\left<x_i,f_i(x_i)\right>\in f_i,1\leq i\leq n$

同理可证对于任意一组 $\left<f_1(x_1),...,f_n(x_n)\right>\in Y^n$ 有且仅有一个 $z\in Z$ 满足 $\left<\left<f_1(x_1),...,f_n(x_n)\right>,z\right>\in g$

总上所述，对于任意 $\left<x_1,...,x_n\right>\in X^n$ 有且仅有一个 $z\in Z$ 满足 $\left<\left<x_1,...,x_n\right>,z\right>\in g\cdot(f_1,...,f_n)$

即根据[[MK175][定义]]易证 $g\cdot(f_1,...,f_n)$ 是[[MK175][函数]]
...
若 $G=\left<V,E\right>$ 是[[MK209][无向图]] 
...
若存在图 $G=\left<V,E\right>$ 且 $v_1\in V$ 若存[[MK217][通道]] $t=\left<v_1,e_1,v_2,e_2,....e_{n-1},v_n\right>$ 满足：

（1） $v_1=v_n=v$

（2） 对于任意 $1\leq i< j\leq n$ 均有 $e_i\ne e_j$

（3） 对于任意 $1\leq i< j< n$ 均有 $v_i\ne v_j$
...
[[MK209][无向图]] $G=\left<V,E\right>$ 是弦图当且仅当任意 $G$ 中的[[MK513][环]] $t=\left<v_1,e_1,v_2,e_2,...,v_{n-1},e_{n-1},v_1\right>$

满足若 $n-1>3$ 则存在 $e\in E$ 以及 $1\leq i<j\leq n-1$ 满足 $e=\{v_i,v_j\}$
