...
若 $|A|<|B|$ 以及 $|B|\leq|C|$ 则有 $|A|<|C|$

***** 证明

假设 $|A|=|C|$ 则根据[[MK327][定理6]]有 $|B|\leq|A|$ ，有根据[[MK328][定理4]]有 $|A|\not<|B|$ 即与条件矛盾
...
假设 $K$ 在 $\mathcal{M}$ 上[[MK445][代数无关]]，则根据[[MK495][存在性]]易证存在 $K^*\subset N$ 满足 $K\subset K^*$ 且 $K^*$ 是 $\mathcal{M}$ 上[[MK144][极大]]的[[MK445][代数无关]]集

则 $K^*$ 亦为[[MK495][超越基]]，根据[[MK503][定理7]]可得 $|S|<|K^*|$ 与[[MK504][性质2]]矛盾，故假设不成立。即 $K$ 在 $\mathcal{M}$ 上[[MK445][代数相关]]
...
（3） $\mathcal{N}_X=\left<N,J_X\right>$ [[MK323][实现]] $\Gamma$
...
（1） [[MK496][超越度]]为[[MK82][有限]]，设 $T=\{t_1,...,t_n\}$ 是[[MK496][超越基]]

（2） [[MK339][特征]]为 $p$ 的[[MK390][代数封闭域]]

假设 $\mathcal{N}$ 是[[~/文档/note/数理逻辑/sllj.org::MK394][饱和模型]]

任取若 $\mathcal{M}$ 是[[MK496][超越度]]为[[MK89][可数]]的[[MK390][代数封闭域]]，其[[MK496][超越基]]为 $S$ ，则根据[[MK500][之前证明]]可得 $\mathcal{M}$ 是[[~/文档/note/数理逻辑/sllj.org::MK394][饱和模型]]，
...
则根据[[~/文档/note/数理逻辑/sllj.org::MK255][完备性]]易证 $\text{Th}(\mathcal{M})=\text{Th}(\mathcal{N})$ 即[[~/文档/note/数理逻辑/sllj.org::MK281][同样]]，又根据[[~/文档/note/数理逻辑/sllj.org::MK438][唯一性]]可得 $\mathcal{M},\mathcal{N}$ [[~/文档/note/数理逻辑/sllj.org::MK274][同构]]设[[~/文档/note/数理逻辑/sllj.org::MK274][同构映射]] $e:M\to N$

任取 $\{s_1,...,s_{n+1}\}\subset S$ ，则有 $\{e(s_1),...,e(s_{n+1})\}\subset N$

根据[[MK504][性质2]]易证 $\{e(s_1),...,e(s_{n+1})\}$ 在 $\mathcal{N}$ 上[[MK445][代数相关]]，

则定义 $\vec{x}=\left<x_1,...,x_{n+1}\right>,e(\vec{s})=\left<e(s_1),...,e(s_{n+1})\right>,\vec{s}=\left<s_1,...,s_{n+1}\right>$

则存在 $f(\vec{x})\in N[\vec{x}]$ 满足 $\mathcal{N}\vDash(f(\vec{x})\hat{=}c_0)[e(\vec{s})]$ ，则根据[[~/文档/note/数理逻辑/sllj.org::MK274][同构]]易证 $\mathcal{M}\vDash(f(\vec{x})\hat{ =}c_0)[\vec{s}]$

即 $\{s_1,...,s_{n+1}\}$ 在 $\mathcal{M}$ 上[[MK445][代数相关]]，显然矛盾。故假设不成立，即 $\mathcal{N}$ 不是[[~/文档/note/数理逻辑/sllj.org::MK394][饱和模型]]
