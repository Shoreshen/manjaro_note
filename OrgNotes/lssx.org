...
对于非空偏序集 $<P,\leq>$ 假设命题不成立，即全序子集有上界，但 $P$ 中不存在[[MK144][极大元]]，即满足

（1） 对于任意[[MK170][全序集]] $<S,\leq>$ 满足 $S\subset P$ 均有上界，即 $\exists b\in P,\forall a\in S\Rightarrow a\leq b$
...
对于的[[MK170][全序子集]] $<S,\leq>$ 有对应的集合 $B(S)$ 必为非空集合
...
首先推断 $C\ne\varnothing$ ，定义 $\{p_1\}=\phi(\{p_0\}),\phi(\{p_0,p_1\})=p_2$ 定义 $S_3=\{p_0,p_1,p_2\}$ 显然 $<S,\leq>$ 是[[MK171][良序集]]
...
根据[[MK161][定理2]]可得 $<U,\leq>$ 亦是[[MK171][良序集]]
...
任意非空集合 $X$ 均存在其上的[[MK143][偏序关系]] $\leq$ 使得 $<X,\leq>$ 是[[MK171][良序关系]]

***** 证明

****** 定义 $<P,\preceq>$ <<MK174>>

定义 $\rho(X)$ 是 $X$ 的[[MK20][幂集]]，定义集合 $P$ 满足对于任意 $<Y,\leq_Y>\in P$ 有：

（1） $Y\in\rho(X)$

（2） $<Y,\leq_Y>$ 是[[MK171][良序关系]]

进一步在 $P$ 上定义关系 $\preceq$ 为 $<Y_1,\leq_{Y_1}>\preceq<Y_2,\leq_{Y_2}>$ 为：

（1） $\leq_{Y_1}\subset\leq_{Y_2}$ 且 $Y_1\subset Y_2$

（2） 在 $<Y_2,\leq_{Y_2}>$ 下 $Y_1$ 是 $Y_2$ 的[[MK155][前段]]

****** 证明 $<P,\preceq>$ 是[[MK143][偏序关系]]

则对于代数结构 $<P,\preceq>$ 有：

（1） 对于任意 $<Y,\leq_Y>\in P$ 有 $Y\subset Y\land \leq_Y\subset\leq_Y$ 且 $Y$ 是 $Y$ 的[[MK155][前段]]，即 $<Y,\leq_{Y}>\preceq<Y,\leq_{Y}>$

（2） 若有 $<Y_1,\leq_{Y_1}>\preceq<Y_2,\leq_{Y_2}>\land<Y_2,\leq_{Y_2}>\preceq<Y_1,\leq_{Y_1}>$

     则根据[[MK16][自反性]]显然 $<Y_1,\leq_{Y_1}>=<Y_2,\leq_{Y_2}>$ 

（3） 若有 $<Y_1,\leq_{Y_1}>\preceq<Y_2,\leq_{Y_2}>\land<Y_2,\leq_{Y_2}>\preceq<Y_3,\leq_{Y_3}>$

     则有 $Y_1\subset Y_3$ 以及 $\leq_{Y_1}\subset\leq_{Y_3}$ 又根据[[MK159][传递性]]可得在 $<Y_3,\leq_{Y_3}>$ 下 $Y_1$ 是 $Y_3$ 的[[MK155][前段]]

综上所述 $<P,\preceq>$ 满足[[MK124][自反性]]，[[MK127][反对称性]]以及[[MK128][传递性]]，是[[MK143][偏序关系]]

****** 证明 $<P,\preceq>$ 有[[MK144][极大元]]

对于任意 $<P,\preceq>$ [[MK153][链]] $C\subset P$ 定义 $Y^*=\bigcup_{<Y,\leq_Y>\in C}Y,\leq_{Y^*}=\bigcup_{<Y,\leq_Y>\in C}\leq_{Y}$

易证 $<Y^*,\leq_{Y^*}>$ 亦为[[MK143][偏序关系]]且任意 $<Y,\leq_Y>\in C$ 有 $<Y,\leq_{Y}>\preceq<Y^*,\leq_{Y^*}>$

则 $<Y^*,\leq_{Y^*}>$ 是 $C$ 在 $<P,\preceq>$ 中的[[MK145][上届]]，根据[[MK162][佐恩引理]]可得 $<P,\preceq>$ 存在[[MK144][极大元]]
...
易证 $<M',\leq_{M'}>$ 亦为[[MK171][良序关系]]且在 $<M',\leq_{M'}>$ 下 $M$ 是 $M'$ 的[[MK155][前段]]

进而有 $<M,\leq_M>\preceq<M',\leq_{M'}>$ 与 $<M,\leq_M>$ 是[[MK144][极大元]]矛盾

故假设不成立 $X=M$ 进而有 $<X,\leq_M>$ 是[[MK171][良序关系]]
