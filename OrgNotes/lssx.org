...
（4） $(\forall x_1(\exists x_2(f_+(x_1,x_2)\hat{=}c_0)))$ 加法逆元
...
[[MK338][结构]] $\mathcal{M}=\left<M,I\right>,\mathcal{N}=\left<N,J\right>$ 是[[MK309][环]]，即 $\mathcal{M}\vDash T_\mathcal{R},\mathcal{N}\vDash T_\mathcal{R}$ ，若存在[[MK175][映射]] $\eta:M\to N$ 满足：

（1） 对于任意 $\{a,b\}\subset M$ 均有 $\eta(I(f_+)(a,b))=J(f_+)(\eta(a),\eta(b))$

（2） 对于任意 $\{a,b\}\subset M$ 均有 $\eta(I(f_\times)(a,b))=J(f_\times)(\eta(a),\eta(b))$

则称 $\eta$ 是 $\mathcal{M}$ 到 $\mathcal{N}$ 的同态[[MK175][映射]]
...
则有 $\mathcal{A}_{\mathcal{R}_e}=\{f_\times,f_+,c_0,c_1\}$ 进一步有 $\mathcal{L}_{\mathcal{A}_{\mathcal{R}_e}}$ 是幺环理论的[[~/文档/note/数理逻辑/sllj.org::MK151][语言]]
...
则有 $\mathcal{A}_\mathbb{F}=\{f_\times,f_+,c_0,c_1\}$ 进一步有 $\mathcal{L}_{\mathcal{A}_\mathcbb{F}}$ 是环理论的[[~/文档/note/数理逻辑/sllj.org::MK151][语言]]

**** 公理<<MK337>>

显然[[MK335][域语言]]包含[[MK361][非零幺环语言]]，即 $\mathcal{A}_{\mathcal{R}_e}\subset\mathcal{A}_\mathbb{F}$ 故[[MK373][非零幺环公理]] $T_\mathcal{R}$ 显然满足 $T_\mathcal{R}\subset\mathcal{L}_{\mathcal{A}_\mathbb{F}}$
...
[[MK338][结构]] $\mathcal{M}$ 是 $\mathcal{N}$ 的[[~/文档/note/数理逻辑/sllj.org::MK282][子结构]]且 $\mathcal{M}\vDash T_\mathbb{F},\mathcal{N}\vDash T_\mathbb{F}$ ，则称 $\mathcal{M}$ 是 $\mathcal{N}$ 的子域， $\mathcal{N}$ 是 $\mathcal{M}$ 的扩张域

****** 生成[[MK341][扩张域]]<<MK393>>

[[MK338][结构]] $\mathcal{M}=\left<M,I\right>$ 是 $\mathcal{N}=\left<N,J\right>$ 的[[MK341][子域]]，若有 $A\subset N-M$
...
对于任意 $n\in\mathbb{N}$ 定义 $\phi_n=\sum_{i=1}^{n}f_\times(x_i,x_{n+1}^i)$ 为 $n$ 次多项式
...
任意[[MK338][域结构]] $\mathcal{M}=\left<M,I\right>$ 的[[MK399][特征]]均为素数

****** 证明

假设 $\mathcal{M}=\left<M,I\right>$ 的[[MK399][特征]] $p=ab$ 则 $\mathcal{M}\vDash(\tau_n\hat{= }c_0)$ 根据[[MK334][加法结合律]]、[[MK334][加法结合律]]易证 $\mathcal{M}\vDash(\tau_n\hat{=}f_+(\tau_a,f_+(...f_+(\tau_a,\tau_a)...)))$
...
（4） $\mathcal{M}[x],\mathcal{N}[x]$ 分别在[[MK348][理想]] $(f(x)),(e(f(x)))$ 上的[[MK369][幺环商环]]分别是 $\mathcal{M}'=\left<M[x]/(f(x)),I'\right>,\mathcal{N}'=\left<N[x]/(f(x)),J'\right>$

则有 $\mathcal{M}',\mathcal{N}'$ [[~/文档/note/数理逻辑/sllj.org::MK274][同构]]

****** 证明

定义映射 $e^*:M[x]/f(x)\to N[x]/e(f(x))$ 满足对于任意 $[f(x)]_\sim=\left[\sum_{i=0}^na_ix^i\right]_\sim\in M[x]/f(x)$ 有 $e^*([f(x)])=\left[\sum_{i=1}^ne(a_i)x^i\right]_\sim\in N[x]/(e(f(x)))$ 
...
对于任意[[MK336][ $n$ 次多项式]] $\phi_n$ 均满足 $(\forall x_1(...(\forall x_n((\lnot(x_n\hat{=}c_0))\to(\exists x_{n+1}(\phi_n\hat{ =}c_0))))...))\in T_\text{ACF}$
