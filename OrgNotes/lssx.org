...
若 $\mathcal{M}=\left<M,I\right>$ 是 $\mathcal{N}=\left<N,J\right>$ 的[[MK341][子域]]且有 $S\subset N$ 在 $\mathcal{M}$ 上[[MK445][代数无关]]

则 $S'\subset N$ 满足在 $\mathcal{M}_{(S)}=\left<M_{(S)},I_{(S)}\right>$ 上[[MK445][代数无关]]的充要条件是 $S\cup S'$ 在 $\mathcal{M}$ 上[[MK445][代数无关]]
...
使得 $f(s_1,...,s_n)=J(c_0)$ 且 $f(\vec{x})\ne I[\vec{x}](c_0)$ 。根据[[MK437][存在性]]不妨设 $f(\vec{x})$ 是[[MK446][最简式]]，显然有 $P\ne\varnothing$
...
（2） $f'(\vec{x})=\sum_{i\in P}\left(\left(a_i\left(\prod_{j\in K_i-S^*}s_j^{k_{i,j}}\right)\right)\left(\prod_{j\in K_i\cap S^*}x_j^{k_{i,j}}\right)\right)$

显然 $f'(\vec{x})\in M_{(S)}[\vec{x}]$ 

*********** 证明 $f'(\vec{x})\ne I_{(S)}[\vec{x}](c_0)$

假设 $f'(\vec{x})=I_{(S)}[\vec{x}](c_0)$ 则 $f'(\vec{x})$ 的[[MK446][最简式]]为 $I(c_0)$ 
...
若 $g(\vec{x})\ne I[\vec{x}](c_0)$ ，则有 $\forall j\in K_i-S^*$ 均有 $s_j\in S$ 显然与 $S$ 在在 $\mathcal{M}$ 上[[MK445][代数无关]]矛盾
...
综上所述，假设不成立，即 $f'(\vec{x})\ne I_{(S)}[\vec{x}](c_0)$

*********** 结论

由于 $f'(s_1,...,s_n)=\sum_{i\in P}\left(\left(a_i\left(\prod_{j\in K_i-S^*}s_j^{k_{i,j}}\right)\right)\left(\prod_{j\in K_i\cap S^*}s_j^{k_{i,j}}\right)\right)=\sum_{i\in P}\left(a_i\left(\prod_{j\in K_i}s_j^{k_{i,j}}\right)\right)=I_{(S)}(c_0)=J(c_0)$

结合 $f'(\vec{x})\ne I_{(S)}[\vec{x}](c_0)$ 显然与 $S'$ 满足在 $\mathcal{M}_{(S)}$ 上[[MK445][代数无关]]矛盾

故假设不成立，即 $S\cup S'$ 在 $\mathcal{M}$ 上[[MK445][代数无关]]

********** 必要

若 $S\cup S'$ 在 $\mathcal{M}$ 上[[MK445][代数无关]]，则假设 $S'$ 在 $\mathcal{M}_{(S)}=\left<M_{(S)},I_{(S)}\right>$ 上[[MK445][代数相关]]

则根据[[MK478][性质3]]可得若 $S\cup S'$ 在 $\mathcal{M}$ 上[[MK445][代数相关]]，即矛盾，故假设不成立

即 $S'$ 在 $\mathcal{M}_{(S)}=\left<M_{(S)},I_{(S)}\right>$ 上[[MK445][代数无关]]
...
则若 $\mathcal{M}$ 关于 $\mathcal{M}'$ 的[[MK438][超越基]] $S$ 与 $\mathcal{N}$ 关于 $\mathcal{N}'$ 的[[MK438][超越基]] $T$ [[MK29][等势]]的充要条件是 $\mathcal{M},\mathcal{N}$ [[~/文档/note/数理逻辑/sllj.org::MK274][同构]]
...
显然 $|T|=|e(T)|$ ，由于 $|S|<|T|$ 根据[[MK503][定理7]]可得 $|S|<|e(T)|$ 根据[[MK505][推论]]可得 $e(T)$ 在 $\mathcal{M}$ 上[[MK445][代数相关]]，即存在：
