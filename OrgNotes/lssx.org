...
定义 $f'(\vec{x})=\sum_{i\in P'}\left(a'_i\left(\prod_{j\in K'_i}x_j^{k'_{i,j}}\right)\right)$ 满足：

（1） 对于任意 $i'\in P'$ 存在 $i\in P$ 满足 $a_i\ne I(c_0)$ ， $K'_{i'}=\left\{j\big|j\in K_i\land k_{i,j}\ne0\right\}$ 以及对于任意 $j\in K'_{i'}$ 均有 $k'_{i',j}=k_{i,j}$

（2） 对于任意 $i'\in P'$ 定义 $I_{i'}=\left\{i\big|K'_{i'}=\left\{j\big|K'_{i'}=\left\{j\big|j\in K_i\land k_{i,j}\ne0\right\}\land\left(\forall j\in K_i\left(k_{i,j}=k'_{i',j}\right)\right)\right\}$ 则定义 $a'_{i'}=\sum_{i\in I_{i'}}a_i$

     其中 $\sum_{i\in I_{i'}}a_i$ 为 $\mathcal{M}$ 上的连续加法 $I(f_+)$

（3） 若 $K'_{i'}=\varnothing$ 则 $a'_i\left(\prod_{j\in K'_i}x_j^{k'_{i,j}}\right)=a'_i$
...
则其[[MK446][最简式]] $f'(\vec{x})=\sum_{i\in P'}\left(a'_i\left(\prod_{j\in K'_i}x_j^{k'_{i,j}}\right)\right)$ 亦为 $\mathcal{M}$ 上关于 $\vec{x}$ 的[[MK436][多项式]]，即 $f'(\vec{x})\in M _\vec{x}$

********* 证明

根据[[MK466][定义]]可得对于任意 $i'\in P'$ 有 $a'_{i'}=\sum_{i\in I_{i'}}a_i$ 其中 $a_i\in M$ ，又由于 $\mathcal{M}$ 是[[MK338][域]]，故有 $a'_{i'}\in M$
...
对于任意[[MK338][域]] $\mathcal{M}=\left<M,I\right>$ 上关于 $\vec{x}=\left<x_0,...,x_{n-1}\right>$ 的[[MK436][多项式]] $f(\vec{x})=\sum_{i=0}^m\left(a_i\left(\prod_{j\in K_i}x_j^{k_{i,j}}\right)\right)$ ，任意 $\mathcal{M}$ 的[[MK341][扩张域]] $\mathcal{N}=\left<N,J\right>$
...
$\mathcal{M}=\left<M,I\right>$ 是 $\mathcal{N}=\left<N,J\right>$ 的[[MK341][子域]]，对于任意 $S\subset N$ 若存在 $S$ 的有限子集 $\{s_1,...,s_n\}\subset S$

满足存在 $\mathcal{M}$ 上关于 $\vec{x}=\left<x_1,...,x_n\right>$ 的[[MK436][多项式]] $f(\vec{x})=\sum_{i\in P}\left(a_i\left(\prod_{j\in K_i}x_j^{k_{i,j}}\right)\right)$ 使得：

（1） $f(\vec{x})$ 的[[MK446][最简式]] $f'(\vec{x})=\sum_{i\in P'}\left(a'_i\left(\prod_{j\in K'_i}x_j^{k'_{i,j}}\right)\right)$ 存在 $i^*\in P'$ 使得 $a'_{i^*}\ne0$

（2） [[MK441][带入值]] $f(s_1,...,s_n)=I(c_0)$

则称 $S$ 代数相关，否则则称 $S$ 代数不相关

****** 性质

若 $\mathcal{M}=\left<M,I\right>$ 是 $\mathcal{N}=\left<N,J\right>$ 的[[MK341][子域]]，若 $S\subset N$ [[MK445][代数无关]]，则对任意[[MK338][域]] $\mathcal{M}$ 上关于 $\vec{x}=\left<x_0,...,x_{n-1}\right>$

的[[MK436][多项式]] $f_1(\vec{x}),f_2(\vec{x})$ 若有其[[MK446][最简式]]不[[MK447][相等]]则对于任意 $\vec{s}=\{s_0,...,s_{n-1}\}\subset S$ 均有 $f_1(\vec{s})\ne f_2(\vec{s})$
...
假设[[MK441][带入值]] $f_1(\vec{s})=f_2(\vec{s})$ 则根据[[MK437][性质3]]可得 $f'_1(\vec{s})=f_1(\vec{s})=f_2(\vec{s})=f'_2(\vec{s})$ 
