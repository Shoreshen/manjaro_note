...
若 $V$ 是一个非空集合， $K$ 为[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org][域]]，若：
...
设 $A,B$ 是 $\mathcal{V}$ 的两组[[MK36][基]]，则对于任意 $a\in A$ 存在唯一 $\{k_1,...,k_n\}\in K$ 且 $k_i\ne 0$ 以及 $B'= \{b_1,...,b_n\}\in B$ 满足 $a=\sum_{i=1}^nk_ib_i$
...
定义 $\varphi$ 的值域为 $\varphi(A)$ 定义 $\varphi^{-1}:\varphi(A)\to\rho(A)$ 满足对于任意 $X\in\varphi(A)$ 均有 $\varphi^{-1}(X)=\left\{a\big|a\in A\land \varphi(a)=X\right\}$

根据[[~/文档/note/离散数学/lssx.org::MK14][选择公理]]可得存在选择函数 $f:\varphi(A)\to A$ ，对于任意 $X_1,X_2\in\varphi(A)$ 满足 $X_1\ne X_2$ 显然 $\varphi^{-1}(X_1)\cap\varphi^{-1}(X_2)=\varnothing$

否则易证与[[MK115][之前证明]]矛盾，故可得 $f$ 是[[~/文档/note/离散数学/lssx.org::MK176][单射]]，即根据[[~/文档/note/离散数学/lssx.org::MK29][定义]]可得 $|\varphi(A)|\leq|A|$

根据[[~/文档/note/离散数学/lssx.org::MK14][选择公理]]可得存在选择函数 $f:\varphi(A)\to A$ 

****** 证明 $|B|\leq|\varphi(A)|$

定义 $B^*=\left\{\{b\}\big|b\in B\right\}$ 即为 $B$ 的单个元素[[~/文档/note/离散数学/lssx.org::MK15][子集]]的集合，



可得不存在 $a\in A$ 满足 $\varphi(a)=X_1,\varphi(a)=X_2$
