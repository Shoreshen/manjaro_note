...
（2） $V$ 上定义了[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $K$ 与 $V$ 中元素的数乘运算即 $K\times V\mapsto V$ ，记作 $\forall k\in K,\forall \vec{a}\in V,\exists \vec{b}\in V\Rightarrow k\vec{a}=\vec{b}$
...
（5） $\forall\vec{a}\in V\Rightarrow 1 \vec{a}=\vec{a}$ 其中 $1$ 是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $K$ 的乘法[[~/文档/note/离散数学/Disc_Math.org::MK311][幺元]]
...
$\mathcal{V}=<V,K,+,\times>$ 是[[MK1][线性空间]]，则定义 $V^n=V\times...\times V$ 即 $n<\infty$ 个 $V$ 的[[~/文档/note/离散数学/Disc_Math.org::MK357][笛卡尔积]]
