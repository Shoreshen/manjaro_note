...
有向量组 $A=\{\vec{a}_1,...,\vec{a}_m\}$ 其中 $\vec{a}_i=<a_{i1},...,a_{in}>$ 不全为零，若满足对于任意 $\{\vec{a}_i,\vec{a}_j\}\in A$

均有 $\sum_{i=1}^na_ib_i=\left[\begin{matrix}a_{i1}&a_{i2}&...&a_{in}\end{matrix}\right]\left[\begin{matrix}a_{j1}\\a_{j2}\\...\\a_{jn}\end{matrix}\right]=0$ 则有 $A$ [[MK8][线性无关]]，其中第二式为[[~/OneDrive/高等代数/Algb-3-Matrix.org::MK5][矩阵乘法]]
...
（2） $\mathcal{V}_1$ 的[[MK36][基]]为 $\{\vec{u}_1,...,\vec{u}_n\}$
...
定义列元素 $\vec{a}_i=<a_{1i},...,a_{mi}>,1\leq i\leq n$ 则称 $\mathcal{V}_{col}<\text{span}(\{\vec{a}_1,...,\vec{a}_n\}),K,+,\cdot>$ 为矩阵的[[MK46][列空间]]

且 $\dim_K(\mathcal{V}_{col})$ ，即列空间的[[MK51][维数]]为矩阵列[[MK19][秩]]

定义行元素 $\vec{a}_i=<a_{i1},...,a_{in}>,1\leq i\leq m$ 则称 $\mathcal{V}_{row}<\text{span}(\{\vec{a}_1,...,\vec{a}_m\}),K,+,\cdot>$ 为矩阵的[[MK46][行空间]]

且 $\dim_K(\mathcal{V}_{row})$ ，及行空间的[[MK51][维数]]为矩阵行[[MK19][秩]]
