...
运算为有序元素对的映射，即 $a\in A,b\in B$ 则根据[[~/文档/note/离散数学/Disc_Math.org::MK153][笛卡尔乘积]] $(a,b)\in A\times B$
...
（5） $\forall\vec{a}\in V\Rightarrow 1 \vec{a}=\vec{a}$ 其中 $1$ 为[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org][域]] $K$ 的乘法[[~/文档/note/离散数学/Disc_Math.org::MK311][幺元]]
...
即 $0\vec{a}=\vec{0},\vec{a}\in V$ 其中 $1$ 是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]] $K$ 的乘法[[~/文档/note/离散数学/Disc_Math.org::MK311][幺元]] 
...
$\mathcal{V}=<V,K,+,\times>$ 是[[MK1][线性空间]]，则定义 $V^n=V\times...\times V$ 即 $n<\infty$ 个 $V$ 的[[~/文档/note/离散数学/Disc_Math.org::MK153][笛卡尔积]]
...
即 $V_1\subset V_2$ 同理可证 $V_2\subset V_1$ 根据[[~/文档/note/离散数学/Disc_Math.org::MK21][自反性]]有 $V_2= V_1$ 即 $\mathcal{V}_1=\mathcal{V}_2$ 
...
即有 $\left\{a+b\big|a\in\text{span}(S_1)\land b\in\text{span}(S_1)\right\}\subset V$ 根据[[~/文档/note/离散数学/Disc_Math.org::MK21][自反性]]
...
若 $S$ 是[[~/文档/note/离散数学/Disc_Math.org::MK42][有限集]]，则有如下证明

若 $S$ 是[[~/文档/note/离散数学/Disc_Math.org::MK44][可数集]]，则定义加法为 $\sum_{i=1}^\infty k_i\vec{\alpha}_i=\lim_{n\to\infty}\sum_{i=1}^n k_i\vec{\alpha}_i$ 下列性质同理可证

若 $S$ 是[[~/文档/note/离散数学/Disc_Math.org::MK324][不可数集]]，则需要定义不可数加法，方可证明相关性质
...
若 $S$ 是[[~/文档/note/离散数学/Disc_Math.org::MK43][无限集]]，则[[MK38][性质5]]可能不成立
...
对于一个[[MK1][线性空间]] $\mathcal{V}$ 定义[[~/文档/note/离散数学/Disc_Math.org::MK185][偏序集]] $<P,\leq>$ 其中 $P$ 的元素是集合 $V$ 的所有子集，偏序关系 $\leq$ 为包含关系，即 

（1） $U\subset V\Rightarrow U\in P$

（2） $U\subset V, L\subset V,U\subset L\Rightarrow U\leq L$

定义 $S\subset P$ 满足若 $L\in S$ 则 $L$ 中的所有元素[[MK22][线性无关]]，易证 $<S,\leq>$ 亦组成[[~/文档/note/离散数学/Disc_Math.org::MK185][偏序集]]

***** 证明任意链有上界

取 $T\subset S$ 为 $S$ 中的任意一条[[~/文档/note/离散数学/Disc_Math.org::MK386][链]]，假设 $T=\left\{B_i\big|i\in I\right\}$ 其中 $I$ 为指标集，定义 $B=\cup_{i\in I}B_i$
...
由于 $T$ 为链，故 $T$ [[~/文档/note/离散数学/Disc_Math.org::MK193][全序]]则有 $\forall 1\leq j_1< j_2\leq n$ 必有 $B_{j_1}\leq B_{j_2}$ 或者 $B_{j_2}\leq B_{j_1}$ 其一成立。

结合偏序关系[[~/文档/note/离散数学/Disc_Math.org::MK172][传递性]]，必有一个元素 $B_{j^*}$ 满足 $\forall 1\leq j\leq n\Rightarrow B_j\leq B_j^*$ 即 $B_j\subset B_{j^*}$
...
根据[[~/文档/note/离散数学/Disc_Math.org::MK390][佐恩引理]] $S$ 中有[[~/文档/note/离散数学/Disc_Math.org::MK186][极大元]]，定义做 $A$ ，从 $V$ 中任取一个元素 $a$ 

若 $a\in A$ 则 $a$ 可由 $A$ 中的元素线性表述

若 $a\not\in A$ 则根据 $<S,\leq>$ 定义的偏序关系，有 $A\subset A\cup\{a\}\Rightarrow A\leq A\cup\{a\}$ 且 $A\ne A\cup\{a\}$

由于 $A$ 是 $S$ 的[[~/文档/note/离散数学/Disc_Math.org::MK186][极大元]]，则 $A\cup\{a\}\not\in S$ 根据 $S$ 的构造 $A\cup\{a\}$ 线性相关。
...
若 $V$ 的一个基包含无限个向量，则 $V$ 的任意两个[[MK36][基]]均[[
...
同理可证 $V'\subset V$ 根据[[~/文档/note/离散数学/Disc_Math.org::MK21][自反性]]有 $V'= V$
...
故根据[[~/文档/note/离散数学/Disc_Math.org::MK21][自反性]]可得 $P=\left\{P(a)\big|a\in V\right\}=M_{1\times n}$ 
...
若线性空间 $\mathcal{V}=<V,K,+,\cdot>,\mathcal{V}'=<V',K,\bar{+},\bar{\cdot}>$ 存在[[~/文档/note/离散数学/Disc_Math.org::MK200][双射函数]] $\sigma:V\rightarrow V'$ 且满足：
...
则 $R$ 为[[~/文档/note/离散数学/Disc_Math.org::MK176][等价关系]]
...
对于任意 $\vec{v}\in V$ 存在[[~/文档/note/离散数学/Disc_Math.org::MK177][等价类]] 即 $\left\{\vec{\alpha}\big|\vec{\alpha} R\vec{v}\right\}=\left\{\vec{\alpha}\big|\vec{\alpha}-\vec{v}\in W\right\}$ 记作 $[\vec{v}]_W$
...
故根据[[~/文档/note/离散数学/Disc_Math.org::MK21][自反性]]有 $W=[\vec{0}]_W$
...
故根据[[~/文档/note/离散数学/Disc_Math.org::MK21][自反性]]有 $[v]_W=[u]_W$
