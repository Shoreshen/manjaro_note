...
且构建确界 $\beta=a_0.a_1a_2a_2...\\$ 根据[[~/文档/note/实变函数/SBHS.org::MK33][定理2]]可得 $\beta\in\mathbb{R}$
...
任意数列 $\{s_n\}$ 均有[[MK29][子列]]满足 $\lim_{i\to\infty}s_{n_i}=A\in\mathbb{R}_e$ 其中 $\mathbb{R}_e$ 为[[~/文档/note/实变函数/SBHS.org::MK143][广义实数]]
...
则由于 $\{s_{n_i}\}$ 是可数集，则 $\{s_{n_v}\},\{s_{n_u}\}$ 必有一个无限集[[~/文档/note/离散数学/Disc_Math.org::MK43][无限集]]
...
是实数系的[[~/文档/note/实变函数/SBHS.org][实数公理-公理定义-完备性公理]]，故成立

亦可根据[[~/文档/note/实变函数/SBHS.org][实数公理-等价定理]]从[[MK9][确界存在定理]]证得
...
若 $V$ 是[[~/文档/note/高等代数/Algb-1-Liner_sys_func.org][数域]] $K$ 上的根据[[~/文档/note/高等代数/Algb-2-Liner_Space.org][线性空间]]，定义 $R$ 为实数，则根据[[~/文档/note/离散数学/Disc_Math.org][关系与函数-函数]]有函数 $\|\cdot\|:V\rightarrow R$ 满足：
...
根据[[~/文档/note/实变函数/SBHS.org::MK39][拓扑定义]]有[[MK15][定理8]]可得（1）、[[P2][定理3]]可得（2）以及[[P3][定理4]]可得（3）
...
则根据[[~/文档/note/实变函数/SBHS.org::MK41][拓扑闭集定义]]可得 $(S^c)^c=S$ 亦为拓扑定义闭集
...
根据[[~/文档/note/实变函数/SBHS.org][拓扑-子集分类-完备集]]，进一步根据闭集定义以及[[MK8][聚点]]定义，
...
对于任意 $a\in A$ 定义 $\delta(a)=\frac{1}{2}d(\{a\},B)$ 其中 $d(a,B)$ 是[[~/文档/note/实变函数/SBHS.org][测度-相关定义-子集距离]]
...
则根据[[~/文档/note/实变函数/SBHS.org][测度-相关定义-度量空间]]三角性可得 $d(a',x)+d(x,b')\geq d(a',b')$
...
此处 $\rightarrow$ 为[[~/文档/note/离散数学/Disc_Math.org::MK60][蕴含]]，由于对于空集 $\vec{x}\in S'$ 永假，
...
若[[MK13][闭集]] $S$ 存在上确界 $\sup(S)$ 则必有 $x\in S$ 满足 $x=\sup(S)$

同理若存在下确界 $\inf(S)$ 则必有 $x\in S$ 满足 $x=\inf(S)$

***** 证明

仅证上确界，根据确界定义可得 $\forall\epsilon>0,\exists x\in S\rightarrow \sup(S)-x<\epsilon$

故取 $\epsilon_1=1$ 则有 $x_1$ ，以此类推取 $\epsilon_n=\min\left(\frac{\epsilon_{n-1}}{2},\sup(S)-x_{n-1}\right)$

若存在 $x_n=\sup(S)$ 则定理证毕，若不存在，则有数列 $\{x_1,x_2,...\}$

且有 $x_i<x_{i+1}$ 且由于 $\sup(S)-x_i<\epsilon_i<\frac{1}{2^i}$

又由于是上确界，故有 $\forall x_i\rightarrow\sup(S)\geq x_i$

综上所述 $\lim_{i\to\infty}x_i=\sup(S)$ 故根据[[JDCYTJ2][充要条件]]可得 $\sup(S)$ 是[[MK8][聚点]]

则根据[[MK13][闭集定义]] $\sup(S)\in S$
...
则其分量函数必定 $\exists \vec{\eta}_i,\vec{\xi}_i\in S, f_i(\vec{\eta}_i) \leq f_i(\vec{x})\leq f_i(\vec{\xi}_i),i=1,2,...,m$

*** 证明

因为 $\lim_{\vec{x}\to \vec{x}_0}\vec{f}(\vec{x})=\vec{f}(\vec{x}_0)$ 故有 $\lim_{\vec{x}\to \vec{x}_0}f_i(\vec{x})=f_i(\vec{x}_0)$

故 $f_i(\vec{x})$ 亦连续，设 $f_i(\vec{x})=y_i,S_i=\{y_i\}$  

根据[[MK4][多元函数有界性定理]]可得映射 $S_i$ 有界闭集，故 $f_i(\vec{x})$ 有界

由于 $f_i(\vec{x})$ 映射结果为实数，故根据确界存在定理，有界必有确界。

上确界定义 $\forall \delta>0, \exists f_i(\vec{x})\Rightarrow f_i(\vec{x})\in \left(\sup\{f_i(\vec{x})\}-\delta,\sup\{f_i(\vec{x})\}\right]$

故根据[[JDCYTJ1][聚点充要条件]]可得 $\sup\{f_i(\vec{x})\}$ 为 $S_i$ 聚点
...
则 $\vec{f}(\vec{x})$ 一致连续，此处添加一元函数[[~/文档/note/数学分析/Chap7Note.org::K3][康托定理]]的链接
...
其中 $\vec{A}\in\mathbb{R}_e^m$ 为[[~/文档/note/实变函数/SBHS.org::MK143][广义实数]] ， $|\vec{h}_n|$ 是点 $\vec{h}_n$ 到原点的[[MK25][距离]]
