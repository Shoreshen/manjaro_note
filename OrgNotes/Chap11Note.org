...
$\exists \delta>0, O(\vec{x},\delta)\subset S$ ，所有内点集合 $S_i$
...
$\forall \delta>0,\exists\vec{a}\ne\vec{x}, \vec{a}\in O(\vec{x},\delta)\Rightarrow \vec{a}\in S'$
...
\begin{aligned}
&\exists \delta>0,\forall\vec{a}\ne\vec{x}, \vec{a}\in O(\vec{x},\delta)\Rightarrow \vec{a}\not\in S\\
...
&\Rightarrow \lim_{\vec{x}\to\vec{x}_0}f(\vec{x})=A\\
\end{aligned}
...
指幂函数，指数函数，对数函数，三角函数，反三角函数，常数经过有限次的有理运算（加、减、乘、除、有理数次乘方、有理数次开方）及有限次函数复合所产生的函数
...
若连续函数 $\vec{f}(\vec{x})=\vec{y}, \vec{x}\in K\subset R^n, S=\{\vec{y}\}\subset R^m$ 若 $K$ 是紧集，则必定 $\exists \vec{\eta}_i,\vec{\xi}_i\in S, f_i(\vec{\eta}_i) \leq f_i(\vec{x})\leq f_i(\vec{\xi}_i),i=1,2,...,m$
...
若连续函数 $\vec{f}(\vec{x})=\vec{y}, \vec{x}\in K\subset R^n, S=\{\vec{y}\}\subset R^m$ 若 $K$ 是紧集，则 $\vec{f}(\vec{x})$ 一致连续，此处添加一元函数[[e:/OneDrive/数学分析/Note/Chapter 7/Chap7Note.org][函数一致连续-康托定理]]的链接
