...
函数 $f(x)=y,x\in\mathbb{R}\land y\in\mathbb{R}$ 则 $\lim_{x\to x_0}f(x_0)=A$ 的充要条件是：

对于任意数列 $\{h_n\}$ 满足 $\lim_{n\to\infty}|h_n|=0$ 均有 $\lim_{n\to\infty}f(x+h_n)=A$

其中 $A\in\mathbb{R}_e$ 为[[~/OneDrive/实变函数/SBHS.org::MK143][广义实数]]

** 证明

根据 $\lim_{n\to\infty}h_n=0$ 可得 $\forall \epsilon>0,\exists N,\forall n>N\rightarrow |h_n|<\epsilon$

仅证 $A\in\mathbb{R}$ 部分，当 $A=+\infty$ 或 $A=-\infty$ 是类似易证

*** 充分

由于 $\lim_{x\to x_0}f(x_0)=A$ 则有 $\forall \delta>0,\exists \tau,\forall |x-x_0|<\tau\rightarrow |f(x)-A|<\delta$

对于任意满足条件的数列 $\{h_n\}$ 亦有 $\forall \epsilon>0,\exists N,\forall n>N\rightarrow |h_n|<\epsilon$

即取 $\epsilon=\tau$ 则对于任意 $n>N$ 均有 $|h_n|<\tau$

又由于 $|x_0+h_n-x_0|=|h_n|$ 即有 $\lim_{n\to\infty}f(x+h_n)=A$

*** 必要

假设 $\lim_{x\to x_0}f(x_0)\ne A$ 则 $\exists \delta>0,\forall \tau,\exists |x'-x_0|<\tau\rightarrow |f(x)-A|>\delta$

则取 $\tau_n=\frac{1}{n}$ 则有 $x'_n$ 满足 $|h_n|=|x'_n-x_0|<\frac{1}{n}\land |f(x+h_n)-A|>\delta$

显然 $\lim_{n\to\infty}|h_n|=0$ 且 $\lim_{n\to\infty}f(x+h_n)\ne A$
