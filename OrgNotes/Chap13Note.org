...
由于 $D_{n-1}$ 有界闭集，故根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-Euclid空间基本定理-Heine-Borel定理]] $D_{n-1}$ 为紧集

根据[[~/文档/note/数学分析/Chap11Note.org][连续函数性质-紧集的连续映射（连续函数）也是紧集]]得出 $\frac{\partial f_i(\vec{t})}{\partial t_j}$ 亦为紧集，同理为有界闭集，则存在 $m_{ij}\leq\frac{\partial f_i(\vec{t})}{\partial t_j}\leq M_{ij}$ 

定义 $M=\max\left\{|M_{ij}|,|m_{ij}|\big|1\leq i\leq n, 1\leq j\leq n-1\right\}$ ，定义 $\vec{t}_1,\vec{t}_2\in D_{n-1}$ 为区域内任意两点

若 $\vec{t}_1,\vec{t}_2$ 之间的连线 $l_{12}\in D_{n-1}$ 则根据[[~/文档/note/数学分析/Chap12Note.org][多元函数中值定理与Taylor公式-中值定理]]有
...
由于 $D_{n-1}$ 为连通集，根据[[~/文档/note/数学分析/Chap11Note.org][连续函数性质-补充连通定义-连通集定义]]不存在 $\vec{t}_1,\vec{t}_2$ 没有通路的情况，所以
...
由于有界闭集，则根据[[~/文档/note/数学分析/Chap11Note.org][Euclid空间基本定理-Heine-Borel定理]]得 $D$ 是紧集，又由于 $f(x,y)$ 连续，根据[[~/文档/note/数学分析/Chap11Note.org][连续函数性质-康托定理]]得函数一直连续，
...
故的 $\forall \epsilon>0,\exists \delta,\forall \lambda<\delta\Rightarrow \sum_{i=1}^n\omega_i\sigma_i<\epsilon$ 即 $\lim_{\lambda\to0}\sum_{i=1}^n\omega_i\sigma_i=0$ 故根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件2]]得黎曼积分可积
...
根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-Euclid空间基本定理-柯西收敛原理]] $\lim_{n\to\infty}\vec{f}_n(t)$ 收敛，定义 $\vec{f}(t)=\lim_{n\to\infty}\vec{f}_n(t)$

故对于任意给定 $t\in[0,1]$ 存在映射 $\vec{f}(t)$

*** 证明面积

定义 $S=\left\{\vec{f}(t)\big|t\in[0,1]\right\}$ 由于 $[0,1]$ 有界闭集，根据[[~/文档/note/数学分析/Chap11Note.org][Euclid空间基本定理-Heine-Borel定理]]得出是紧集。

又根据据[[~/文档/note/数学分析/Chap11Note.org][连续函数性质-紧集的连续映射（连续函数）也是紧集]]得出 $S$ 也为紧集，同理 $S$ 有界闭集。 

则根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-开集与闭集-闭集定义]]故有 $S$ 包含聚点。
...
故对于 $\vec{a}$ 的任意领域均包含 $S$ 的点，故根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-开集与闭集-点的定义]] $\vec{a}$ 为 $S$ 的聚点。又由于 $S$ 闭集，故 $\vec{a}\in S$
...
其中 $L$ 为达布大和， $l$ 为达布小和，证明同[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件1]]
...
证明同[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件2]]
...
根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件2]]得 $\forall P:\lim_{\lambda \to 0} \sum_{i=1}^n \omega_i v_i = 0$ 即 $\forall\epsilon>0,\exists \delta, \forall \lambda<\delta\Rightarrow \forall P: \sum_{i=1}^n \omega_iv_i<\epsilon$
...
根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件2]]得 $\lim_{\lambda\to0}\sum_{i=1}^n v_i \sigma_i=0$ 对于任意划分，其中 $\omega_i$ 为区域 $D_i$ 上函数的振幅
...
又根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件2]]得 $f(\vec{x})$ 在 $D_1,D_2$ 上分别可积
...
且根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件2]]得 $\lim_{\lambda\to0}\sum_{i=1}^n \omega_i v^1_i=0,\lim_{\lambda\to0}\sum_{i=1}^n \omega_i v^2_i=0$
...
故根[[~/文档/note/数学分析/Chap11Note.org][据构造Euclid空间-Euclid空间基本定理-康托闭区域套定理]]，必有唯一一点 $\vec{\xi}\in \cap_{n=1}^\infty D^n_i$ ，且有 $\eta\in[m^k_i,m^k_j],\forall k\in N^+$
...
又因为 $\int_D f(\vec{x})dv$ 可积，则根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件1]]得 
...
（2） 由于 $T_k(\vec{x})$ 有连续偏导，故根据[[~/文档/note/数学分析/Chap12Note.org][偏导数与全微分-可微条件]]得出可微，根据[[~/文档/note/数学分析/Chap12Note.org][偏导数与全微分-欧几里得空间连续，可导，可微及方向导数关系-可微与连续]]推出<<MK12>> $T_k(\vec{x})$ 连续

（3） $[T_k(b_k,\vec{x}_k)-T_k(a_k,\vec{x}_k)]$ 根据结论（2）也连续 

（4） 由于连续映射，且 $A^k$ 为有界集闭集，

      根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-Euclid空间基本定理-Heine-Borel定理]]及[[~/文档/note/数学分析/Chap11Note.org][连续函数性质-紧集的连续映射（连续函数）也是紧集]]得出 $[T_k(\vec{x}_u)-T_k(\vec{x}_l)]$ 有界
...
又因为结论（2）可得 $T_k(u,\vec{\xi})$ 对于 $u$ 在 $[a,b]$ 连续，根据条件（1）得出 $u$ 在 $(a,b)$ 可导，故根据[[~/文档/note/大学物理/Note/DiffEq.org][导数基本知识-拉格朗日中值定理]]可得
...
若 $D_i$ 中包含 $D$ 的边界点，则根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-开集与闭集-点的定义-边界点]]的定义 $D_i$ 中包含不属于 $D$ 的点
...
根据条件（4）有 $D$ 为有界闭集，根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-Euclid空间基本定理-Heine-Borel定理]] $D$ 紧集

根据[[~/文档/note/数学分析/Chap11Note.org][连续函数性质-紧集的连续映射（连续函数）也是紧集]]得 $T(D)$ 紧集，同理 $T(D)$ 有界闭集 

故 $f(\vec{t})$ 为有界闭集上的连续函数，根据[[~/文档/note/数学分析/Chap11Note.org][连续函数性质-最大值定理]]存在 $m'\leq f(\vec{t})\leq M',\vec{t}\in T(D)$ 同理存在 $m''\leq f\left(\vec{T}(\vec{x})\right)\left|\det\left(\frac{\partial \vec{T}(\vec{x})}{\partial\vec{x}}\right)\right|\leq M''$
...
定义 $\vec{F}(\vec{x}_0)=\vec{T}\left(\vec{G}(\vec{x}_0)\right)\in R^n, \vec{x}_0\in R^n$ 满足[[~/文档/note/数学分析/Chap12Note.org][多元复合函数求导-链式法则]]的两个条件，即
...
故根据[[~/文档/note/数学分析/Chap12Note.org][隐函数-逆映射定理]]在 $O(\vec{t}^1_0,\rho_1)$ 内有
...
同理根据[[~/文档/note/数学分析/Chap12Note.org][隐函数-逆映射定理]]在 $O(\vec{t}^2_0,\rho_2)$ 内
...
同理根据[[~/文档/note/数学分析/Chap12Note.org][隐函数-逆映射定理]]在 $O(\vec{t}^3_0,\rho_3)$ 内
...
根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-Euclid空间基本定理-Heine-Borel定理]] $D$ 为紧集，

根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-Euclid空间基本定理-Heine-Borel定理-前提定义-紧集]]给出的定义，任意无限开覆盖必有有限子覆盖。
...
由于 $f(x)>0,x\in[a,b]$ 且根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定理1]]得 $\sum_{i=1}^n f(m_i)\Delta x_i\leq\underline{\int_a^b}f(x)dx\right$
...
则根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-区间可加]]，在某一给定划分满足条件的划分 $P: a=x_0<x_1<x_2<...<x_{n-1}<x_n=b,\lambda<\delta$ 成立 
...
又因为 $f(x)$ 在 $[a,b]$ 上可积，根据[[~/文档/note/数学分析/Chap7Note.org][定积分可积定理-定积分可积充要条件1]] $\underline{\int_a^b}f(x)dx=\int_a^bf(x)dx$
...
****** 使用[[~/文档/note/数学分析/Chap10Note.org][函数项级数-一致收敛性质-Dini定理]]
...
故根据[[~/文档/note/数学分析/Chap10Note.org][函数项级数-一致收敛性质-Dini定理]]函数 $h_n(x)$ 在定义域 $x\in[a,b]$ 上一致收敛

又根据[[~/文档/note/数学分析/Chap10Note.org][函数项级数-一致收敛性质-可积性]] $\lim_{n\to\infty}\left\{\int_a^b h_n(x)dx\right\}=\int_a^b \left\{\lim_{n\to\infty}h_n(x)\right\}dx=0$
...
则根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-保序性]]及[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-线性]]有：
...
故根据之前推导和[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-保序性]]及[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-线性]]有：
...
（3） $f_n(x),f(x)$ 在 $[a,b]$ 上均可积，故根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-线性]] $\int_a^bf_n(x)-f(x)dx$ 也可积，
  
     又根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-绝对可积]] $\int_a^b|f_n(x)-f(x)|dx$ 也可积 
...
则根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-保序]]可得 $\int_a^b-|f_n(x)-f(x)|dx\leq \int_a^b f_n(x)-f(x)dx\leq\int_a^b|f_n(x)-f(x)|dx$
...
再根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-线性]]和极限四则运算有 $\lim_{n\to\infty}\int_a^b f_n(x)-f(x)dx=0=\lim_{n\to\infty}\int_a^b f_n(x)dx-\lim_{n\to\infty}\int_a^b f(x)dx$ 
...
（3） 定义 $d(\Gamma)=inf\left\{|\vec{x}|\big|\vec{x}\in\Gamma\right\}$ 其中 $|\vec{x}|$ 为向量 $\vec{x}$ 到原点的距离，根据[[~/文档/note/数学分析/Chap11Note.org][构造Euclid空间-定义距离]]计算
...
又由于 $0\leq\int_{D_n} f(\vec{x})d\vec{x}\leq M$ 故数列 $\left\{\int_{D_n}f(\vec{x})d\vec{x}\right\}$ 有界，根据[[~/文档/note/数学分析/Chap11Note.org][实数系基本定理-Bolzano-Weierstrass定理]]，有界数列必有收敛子列
...
则根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-线性]]
...
定义 $K>a_i,F(x_i)=\int_{a_{i+1}}^\infty...\int_{a_n}^\infty f(\vec{x})dx_n...dx_{i+1},G(x_i)=\int_{a_{i+1}}^\infty...\int_{a_n}^\infty g(\vec{x})dx_n...dx_{i+1}$ ，根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-线性]]及给定条件
...
（1） $\forall u\in N^+$ 定积分 $\int_{a_i}^{A_i^u} \psi_n(x_i)dx_i$ 可积（根据[[~/文档/note/数学分析/Chap8Note.org][反常积分定义-边界无穷积分]]及反常累次积分可积条件）
...
（2） 累次积分 $\int_0^\infty\int_0^\infty e^{-(x^2+y^2)}dydx=\int_0^\infty e^{-x^2}dx\int_0^\infty e^{-y^2}dy=\left(\int_0^\infty e^{-x^2}dx\right)^2$
      ...
      由于 $0<e^{-x^2}<e^{-x},x\in[1,\infty]$ 且根据[[~/文档/note/数学分析/Chap7Note.org][牛顿莱布尼茨公式]] $\int_1^\infty e^{-x} dx=-\frac{1}{e^\infty}-\left(-\frac{1}{e}\right)<\infty$

      故根据保序性 $\int_1^\infty e^{-x^2}dx<\int_1^\infty e^{-x}dx<\frac{1}{e}$ 有界，且函数 $f(A)=\int_0^A e^{-x^2}dx,A>0$ 为单调递增函数

      故 $\int_1^\infty e^{-x^2}dx$ 收敛，又由于 $\int_0^1 e^{-x^2}dx$ 收敛，故根据[[~/文档/note/数学分析/Chap7Note.org][定积分基本性质-区间可加]]  $\int_0^\infty e^{-x^2}dx$ 收敛
...
显然其敛散性取决于 $\int_a^R\frac{1}{r^{p-n+1}}dr$ 根据[[~/文档/note/数学分析/Chap7Note.org][牛顿莱布尼茨公式]]
