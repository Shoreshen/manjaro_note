...
则有 $f(x)=\left(I_{|M_1}\left(\sum\right)\right)_{i=1}^m(k_i\sigma_1)x^i$ 亦为 $\mathcal{M}_1$ 上的[[~/文档/note/抽象代数/cxds.org::MK436][多项式]]，其中 $\left(I_{|M_1}\left(\sum\right)\right)$ 为 $I_{|M_1}(f_+)$ 的连加

则有 $f(x)$ 对 $\sigma$ 的[[~/文档/note/抽象代数/cxds.org::MK441][带入值]]为 $I(c_0)$ 即[[MK65][一元结构]] $\mathcal{M}_\sigma=\left<M_\sigma,I\right>$ 的[[MK61][加法零元]]

***** 证明

显然 $f(x)$ 对 $\sigma$ 的[[~/文档/note/抽象代数/cxds.org::MK441][带入值]]为[[MK65][一元结构]] $\mathcal{M}_\sigma=\left<M_\sigma,I\right>$ 上的运算，又根据[[MK70][推论2]]可得 $\mathcal{M}_\sigma$ 与 $G$ 的[[~/文档/note/高等代数/Algb-3-Matrix.org::MK75][方阵环]] $\mathcal{M}_{G}=\left<M_{G},I_{G}\right>$ [[~/文档/note/数理逻辑/sllj.org::MK274][同构]]
...
则根据[[~/文档/note/数理逻辑/sllj.org::MK275][引理]]可得 $$f(\sigma)=e^{-1}(e(f(\sigma)))=e^{-1}\left(\left(I_G\left(\sum\right)\right)_{i=1}^mI_G(f_\times)\left(k_iI_G(c_1),G^i\right)\right)=I_G(c_0)\right)=e^{-1}(I_G(c_0))=I(c_0)$$
