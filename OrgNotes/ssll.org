...
任意[[~/文档/note/数理逻辑/sllj.org::MK151][可数语言]] $\mathcal{L}_\mathcal{A}$ 及其[[~/文档/note/数理逻辑/sllj.org::MK144][语句]]集合 $T$ 满足：

（1） $\mathcal{L}_\mathcal{A}$ 下[[MK118][逻辑公理关系]] $\text{LAX}$ 为[[MK81][递归关系]]，

（2） $\mathcal{L}_\mathcal{A}$ 是[[MK1][ $\mathcal{L}_{\mathcal{A}_N}$ ]]的[[~/文档/note/数理逻辑/sllj.org::MK152][扩充语言]]

（3） $T$ 是[[MK141][有效表达式]]集合

（4） 存在 $\mathfrak{N}$ 在 $\mathcal{L}_\mathcal{A}$ 下的[[~/文档/note/数理逻辑/sllj.org::MK295][扩充结构]] $\mathfrak{N}_T$ 满足 $\mathfrak{N}_T\vDash T$

则 $T$ 不[[~/文档/note/数理逻辑/sllj.org::MK264][完全]]

*** 证明

根据[[MK178][性质]]可得[[MK174][ $\text{PRF}_T$ ]]是[[MK81][递归关系]]，则根据[[MK179][性质]]可得[[MK180][ $\text{PRV}_T$ ]]亦为[[MK81][递归关系]]

则根据[[MK77][定理1]]可得存在[[MK1][ $\mathcal{L}_{\mathcal{A}_N}$ ]]下的 $\phi_{\text{P}_T}(x_1,x_2,x_3)$ 为[[MK74][特征函数]] $\chi_{\text{PRV}_T}$ 的[[MK102][等价表达]]

定义 $\phi=\left(\lnot\left(\exists x_1\left(\exists x_3\left(\phi_{\text{P}_T}\land (x_3\hat{=}f_s(c_0))\right)\right)\right)\right)$ 根据[[MK181][不动点定理]]可得 $\sigma\in\mathcal{L}_{\mathcal{A}_N}$ 满足 $T_N\vdash\left(\sigma\leftrightarrow\phi\left(\left<x_2\right>,\left<\tau_{\lceil\sigma\rceil}\right>\right)\right)$

**** $\mathfrak{N}\vDash\sigma$ 充要条件为 $T\not\vdash\sigma$

***** 充分

若 $\mathfrak{N}\vDash\sigma$ 显然有 $\mathfrak{N}\vDash T_N$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK257][完备性]]可得 $\mathfrak{N}\vDash \left(\sigma\leftrightarrow\phi\left(\left<x_2\right>,\left<\tau_{\lceil\sigma\rceil}\right>\right)\right)$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]显然 $\mathfrak{N}\vDash\phi\left(\left<x_2\right>,\left<\tau_{\lceil\sigma\rceil}\right>\right)$ 

则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]易证任意 $a\in\mathbb{N}$ 均有 $\mathfrak{N}\not\vDash\left(\exists x_3\left(\phi_{\text{P}_T}\land (x_3\hat{=}f_s(c_0))\right)\right)[a]$

即根据 $\phi_{\text{P}_T}(x_1,x_2,x_3)$ 为[[MK74][特征函数]] $\chi_{\text{PRV}_T}$ 的[[MK102][等价表达]]、[[MK180][ $\text{PRV}_T$ 定义]]以及据[[~/文档/note/数理逻辑/sllj.org::MK196][证明定义]]易证 $T\not\vdash\sigma$

***** 必要

若 $\mathfrak{N}\not\vDash\sigma$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK257][完备性]]可得 $\mathfrak{N}\vDash \left(\sigma\leftrightarrow\phi\left(\left<x_2\right>,\left<\tau_{\lceil\sigma\rceil}\right>\right)\right)$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]显然 $\mathfrak{N}\not\vDash\phi\left(\left<x_2\right>,\left<\tau_{\lceil\sigma\rceil}\right>\right)$ 

根据[[~/文档/note/数理逻辑/sllj.org::MK462][双重否定]]可得 $\mathfrak{N}\vDash\left(\exists x_1\left(\exists x_3\left(\phi_{\text{P}_T}\left(\left<x_2\right>,\left<\tau_{\lceil\sigma\rceil}\right>\right)\land (x_3\hat{=}f_s(c_0))\right)\right)\right)$ 

则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]存在 $a\in\mathbb{N}$ 使得 $\mathfrak{N}\vDash\left(\exists x_3\left(\phi_{\text{P}_T}\left(\left<x_2\right>,\left<\tau_{\lceil\sigma\rceil}\right>\right)\land (x_3\hat{=}f_s(c_0))\right)\right)$

根据 $\phi_{\text{P}_T}(x_1,x_2,x_3)$ 为[[MK74][特征函数]] $\chi_{\text{PRV}_T}$ 的[[MK102][等价表达]]、[[MK180][ $\text{PRV}_T$ 定义]]以及据[[~/文档/note/数理逻辑/sllj.org::MK196][证明定义]]

易证可解析 $a$ 得到[[~/文档/note/数理逻辑/sllj.org::MK196][证明]] $\left<\phi_1,...,\phi_n\right>$ 满足 $\beta(\mathfrak{N},a,i)=\lceil\phi_i\rceil$ 且 $\lceil\phi_n\rceil=\lceil\sigma\rceil$

根据[[MK182][性质2]]可得 $\phi_n=\sigma$ 即 $\left<\phi_1,...,\phi_n\right>$ 是 $\sigma$ 在 $\mathcal{L}_\mathcal{A}$ 下 $T$ 的[[~/文档/note/数理逻辑/sllj.org::MK196][证明]]，即 $T\vdash\sigma$

**** 证明结论

假设 $T\vdash\sigma$ 则根据 $\mathfrak{N}_T\vDash T$ 以及[[~/文档/note/数理逻辑/sllj.org::MK257][完备性]]可得 $\mathfrak{N}_T\vDash\sigma$ 由于 $\sigma\in\mathcal{L}_{\mathcal{A}_N}$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK296][定理1]]可得 $\mathfrak{N}\vDash\sigma$

则根据上一步可得 $T\not\vdash\sigma$ 矛盾。故可得 $T\not\vdash\sigma$

同理根据上一步有 $\mathfrak{N}\vDash\sigma$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK296][定理1]]可得 $\mathfrak{N}_T\vDash\sigma$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得 $\mathfrak{N}_T\not\vDash(\lnot\sigma)$ 

由于 $\mathfrak{N}_T\vDash T$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK257][完备性]]反正易得 $T\not\vdash(\lnot\sigma)$ 即 $T$ 不[[~/文档/note/数理逻辑/sllj.org::MK264][完全]]
