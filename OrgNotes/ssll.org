...
（2） 存在 $a_1\in\mathbb{N}$ 满足 $\prec SN(f_s),SN((),a_1,SN())\succ$ 且 $\left<a_1,b\right>\in\text{Free}$
...
（2） $b=\prec SN((),a,SN(\to),b,SN())\succ$

其中 $\text{Frml}$ 为[[MK120][判断关系]]
...
（5） $R_{\tau,f_i},R_{\phi,p_i}$ 均为[[MK96][拆分关系]]
...
c&a\in\text{Vrble}\land b=a\\
\prec SN(f_1),SN((),(u)_{((a)_3},SN(,),(u)_{(a)_5},...,SN(,),(u)_{(a)_{2\pi_F(f_1)+1}},SN())\succ&a\in R_{\tau,f_1}\\
...\\
\prec SN(f_{n_f}),SN((),(u)_{(a)_3},SN(,),(u)_{(a)_5},...,SN(,),(u)_{(a)_{2\pi_F(f_{n_f})+1}},SN())\succ&a\in R_{\tau,f_{n_f}}\\
\prec SN(p_1),SN((),(u)_{(a)_3},SN(,),(u)_{(a)_5},...,SN(,),(u)_{(a)_{2\pi_P(p_1)+1}},SN())\succ&a\in R_{\phi,p_1}\\
...\\
\prec SN(p_{n_p}),SN((),(u)_{(a)_3},SN(,),(u)_{(a)_5},...,SN(,),(u)_{(a)_{2\pi_P(p_{n_p})+1}},SN())\succ&a\in R_{\phi,p_{n_p}}\\
\prec SN((),(u)_{(a)_2},a_1,SN(\hat{=}),(u)_{(a)_4},SN())\succ&a\in R_{\tau,=}(a)\\
\prec SN((),SN(\lnot),(u)_{(a)_3},SN())\succ&a\in R_{\phi,\lnot}(a)\\
\prec SN((),(u)_{(a)_2},a_1,SN(\to),(u)_{(a)_3},SN())\succ&a\in R_{\phi,\to}(a)\\
\prec SN((),SN(\forall),(a)_3,SN((),(u)_{(a)_4},SN()),SN())\succ&a\in R_{\phi,\forall}(a)\land (a)_3\in\text{Vrble}\\
a&else
\end{cases}
\end{aligned}
...
对于任意[[~/文档/note/数理逻辑/sllj.org::MK151][可数语言]] $\mathcal{L}_\mathcal{A}$ 的[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]集合 $\Gamma$ 若存在 $\mathcal{L}_{\mathcal{A}_{N}}$ 上的[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]] $\phi(x_1)$ 满足 $\left<\mathfrak{N},v\right>\vDash\phi$ 的充要条件是存在 $\psi\in\Gamma\cup\mathbb{L}$

使得 $\lceil\psi\rceil=v(x_1)$ ，则称 $\Gamma$ 为有效[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]集合。其中 $\mathbb{L}$ 表示[[~/文档/note/数理逻辑/sllj.org::MK197][逻辑公理]]
...
则根据[[MK77][定理1]]可得存在[[MK102][等价表达式]] $\phi_f(x,y)$ 满足 $\left<\mathfrak{N},v\right>\vDash\phi_f$ 的充要条件是 $f(v(x))=v(y)$

显然 $f(v(x_1))=1$ 的充要条件是存在 $\psi\in\Gamma\cup\mathbb{L}$ 且 $v(x_1)=\lceil\psi\rceil$

则定义 $\phi=(\phi_f\land(y\hat{=}f_s(c_0)))$ 则显然 $\left<\mathfrak{N},v\right>\vDash\phi$ 的充要条件是 $f(v(x))=1$ 即存在 $\psi\in\Gamma\cup\mathbb{L}$ 且 $v(x_1) =\lceil\psi\rceil$
...
******** $((\phi(\left<x_1\right>,\left<c_0\right>)\land(\forall x_1(\phi\to\phi(\left<x_1\right>,\left<f_s(x_1)\right>))))\to(\forall x_1\phi))$
...
[[MK95][ $\text{Vrble}$ ]]、[[MK128][ $\text{Cnst}$ ]]均为[[MK81][递归关系]]
...
假设有[[~/文档/note/数理逻辑/sllj.org::MK109][函数符号]] $f_1,...,f_{n_f}$ 则定义 $g(u,a)=\begin{cases}\prod_{i=2}^{\pi_F(f_1)}\beta(\mathfrak{N},u,(a)_{2i-1})&a\in R_{\tau,f_1}\\...\\\prod_{i=2}^{\pi_F(f_{n_f})}\beta(\mathfrak{N},u,(a)_{2i-1})&a\in R_{\tau,f_{n_f}}\\1&a\in\text{Vrble}\lor\text{Cnst}\\0&else\end{cases}$
...
\begin{aligned}
f(a)=\begin{cases}
\prod_{i=2}^{\pi_F(f_1)}f((a)_{2i-1})&a\in R_{\tau,f_1}\\...\\\prod_{i=2}^{\pi_F(f_{n_f})}f((a)_{2i-1})&a\in R_{\tau,f_{n_f}}\\1&a\in\text{Vrble}\lor\text{Cnst}\\0&a\in R^*\end{cases}
\end{aligned}
...
若[[MK92][ $\text{Term}$ ]]、[[MK94][ $\text{AFrml}$ ]]均为[[MK81][递归关系]]，则所有[[MK162][基础判断]]均为[[MK81][递归关系]]
...
若[[MK92][ $\text{Term}$ ]]、[[MK94][ $\text{AFrml}$ ]]均为[[MK81][递归关系]]，则[[MK118][逻辑公理关系]]（1）~（12）是[[MK81][递归关系]]
...
若[[MK92][ $\text{Term}$ ]]、[[MK94][ $\text{AFrml}$ ]]均为[[MK81][递归关系]]，则 $\text{LAX}_0$ 是[[MK81][递归关系]]
...
若[[MK92][ $\text{Term}$ ]]、[[MK94][ $\text{AFrml}$ ]]均为[[MK81][递归关系]]，则 $\text{GLB},\text{LAX}$ 是[[MK81][递归关系]]
