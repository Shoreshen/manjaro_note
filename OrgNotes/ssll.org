...
（3） $\text{LenI}(s,I)=(\exists y(\exists \gamma(b'(\gamma,s,I)\land b'(y,\gamma,c_0)\land(y\hat{=}2))))$ 即 $\left<\mathfrak{N},v\right>\vDash\text{LenI}(s,l)$ 的充要条件是 $\beta(\mathfrak{N},\beta(\mathfrac{N},v(s),v(I)),0)=2$
...
（16） $\text{VldOut}_0(s,m,\vec{a},x_2)=\left(\exists x_1\left(\text{IsQi}(\vec{a},x_1)\land\left(\bigwedge_{w=1}^k(\exists a'(\text{ElmIA}(s,m,a',\tau_w)\land\text{G}(\vec{a},x_1,x_2,\tau_w,a')))\right)\right)\right)$
      ...
      其中 $i$ 满足 $v(\vec{a})\in Q_i$ 且 $1\leq w\leq k$
...
（17） $\text{VldOut}_1(s,m,x_1,x_2)=\left(\exists \vec{b}\left(\text{VecIB}(s,m,\vec{b})\land\text{H}\left(\vec{b},x_1,x_2\right)\right)\right)$ 即 $\left<\mathfrak{N},v\right>\vDash\text{VldOut}_1(s,m,x_1,x_2)$ 的充要条件
...
（18） $\text{VldOut}_2(s,I,m,\vec{a},x_1)=(\exists b(\text{ElmIB}(s,I,b,x_1)\land(\exists x_2(\text{IsQi}(\vec{a},x_2)\land\text{VldOut}_1(s,m,x_2,b)))))$ 即 $\left<\mathfrak{N},v\right>\vDash\text{VldOut}_2(s,I,m,\vec{a},x_1)$ 的充要条件

      即 $\beta(\mathfrak{N},\beta(\mathfrak{N},\beta(\mathfrak{N},v(s),v(I)),1),v(x_1))=h_{i}\left(\beta(\mathfrak{N},\beta(\mathfrak{N},\beta(\mathfrak{N},v(s),v(m)),1),1),...,\beta(\mathfrak{N},\beta(\mathfrak{N},\beta(\mathfrak{N},v(s),v(m)),1),k)\right)$
...
最终定义 $\phi(\vec{a},x_1)=\left(\exists s\left(\exists l_0\left(\text{Len}(s,l_0)\land\text{Fnl}(\vec{a},x_1)\land\left(\forall I\left(\text{InRngA}(I,\tau_1,l_0)\to(\text{VldIn}(s,I)\land\text{VldOut}(s,I))\right)\right)\right)\right)\right)$
...
根据[[MK111][推论]]以及[[MK116][用到的拆分关系]]可得以下 $1$ 元[[~/文档/note/离散数学/lssx.org::MK514][关系]]均存在[[MK105][等价表达]] $\phi_*$ 且
