...
*** [[~/文档/note/离散数学/lssx.org::MK175][函数]]<<MK102>>

对于 $n$ 元[[~/文档/note/离散数学/lssx.org::MK175][函数]] $f:M^n\to M$ 以及任意 $\mathcal{L}_{\mathcal{A}_{N}}$ 的任意[[~/文档/note/数理逻辑/sllj.org::MK153][结构]] $\mathcal{M} =\left<M,I\right>\vDash T_{N}$ 若存在表达式 $\phi(x_1,...,x_n,x_{n+1})$

满足对于 $\mathcal{M}$ 的任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash\phi$ 的充要条件是 $f(v(x_1),...,v(x_n))=v(x_{n+1})$

则称 $\phi$ 是 $f$ 在 $\mathcal{M}$ 上的等价表达式。为方便，可记作 $\phi(\vec{x},y)$

**** 性质1（复合）<<MK103>>

若有任意 $f:M^m\to M$ 以及 $g_1,...,g_k:M^n\to M$ 在 $\mathcal{L}_{\mathcal{A}}$ [[~/文档/note/数理逻辑/sllj.org::MK153][结构]] $\mathcal{M} =\left<M,I\right>$ 上均有[[MK102][等价表达式]]

则其[[~/文档/note/离散数学/lssx.org::MK515][复合]] $f\cdot(g_1,...,g_k)$ 在 $\mathcal{M} =\left<M,I\right>$ 上亦有[[MK102][等价表达式]]
...
若有 $\left<\mathcal{M},v\right>\vDash\phi$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得存在 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\eta$ 满足：

（1） $\eta(u)=v(u),u\not\in\{y_1,...,y_k\}$

（2） $\left<\mathcal{M},\eta\right>\vDash\left(\bigwedge_{i=1}^m\phi_{g_i}(\vec{x},y_i)\land\phi_f(\vec{y},z)\right)$

又根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]有

（3） $\left<\mathcal{M},\eta\right>\vDash\phi_{g_i}(\vec{x},y_i),1\leq i\leq k$

（4） $\left<\mathcal{M},\eta\right>\vDash\phi_f(\vec{y},z)$

根据 $\phi_{g_i}(\vec{x},y_i)$ 是 $g_i$ 的[[MK102][等价表达式]]可得 $g_i(v(x_1),...,v(x_n))=\eta(y_i)$

根据 $\phi_f(\vec{y},z)$ 是 $f$ 的对应[[MK139][表达式]]可得 $f(\eta(y_1),...,\eta(y_n))=v(z)$
...
则根据 $\phi_{g_i}(\vec{x},y_i)$ 是 $g_i$ 的对应[[MK139][表达式]]可得 $\left<\mathcal{M},\eta\right>\vDash\phi_{g_i}(\vec{x},y_i),1\leq i\leq k$

又根据 $\phi_f(\vec{y},z)$ 是 $f$ 的对应[[MK139][表达式]]可得 $\left<\mathcal{M},\eta\right>\vDash\phi_f(\vec{y},z)$

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $\left<\mathcal{M},\eta\right>\vDash\left(\bigwedge_{i=1}^m\phi_{g_i}(\vec{x},y_i)\land\phi_f(\vec{y},z)\right)$ 又根据 $\eta$ 定义以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]有 $\left<\mathcal{M},v\right>\vDash\phi$

*** [[~/文档/note/离散数学/lssx.org::MK514][关系]]<<MK105>>

对 $n$ 元[[~/文档/note/离散数学/lssx.org::MK514][关系]] $R\subset M$ 以及任意 $\mathcal{L}_{\mathcal{A}_{N}}$ 的任意[[~/文档/note/数理逻辑/sllj.org::MK153][结构]] $\mathcal{M} =\left<M,I\right>\vDash T_{N}$ 若存在表达式 $\phi(x_1,...,x_n)$

满足对于 $\mathcal{M}$ 的任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 有 $\left<\mathcal{M},v\right>\vDash\phi$ 的充要条件是 $\left<v(x_1),...,v(x_n)\right>\in R$

则称 $\phi$ 是 $R$ 在 $\mathcal{M}$ 上的等价表达式。为方便，可记作 $\phi(\vec{x})$
...
若函数 $f:\mathbb{N}^n\to\mathbb{N}$ 对于任意 $\vec{a}=\left<a_1,...,a_k\right>\in\mathbb{N}$ 有如下定义：
...
（1） $h_i:\mathbb{N}^n\to\mathbb{N}$ 与 $g_{i,j}:\mathbb{N}^n\to\mathbb{N}$ 存在[[MK102][等价表达式]]

（2） $Q_1,...,Q_{n+m}$ 形成 $\mathbb{N}^n$ 存在[[MK105][等价表达式]]，且为[[MK81][递归关系]]

则称 $f$ 为分段递归函数。若 $n$ 元[[~/文档/note/离散数学/lssx.org::MK514][关系]] $R$ 其[[MK74][特征函数]]是分段递归函数，则称 $R$ 为分段递归关系
...
则存在[[MK139][表达式]] $\phi(x_1,...,x_{k+1})$ 使得对于[[MK27][自然数标准模型]] $\mathfrak{N}$ 及其任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$

有 $\left<\mathfrak{N},v\right>\vDash\phi$ 的充要条件是 $f(v(x_1),...,v(x_k))=v(x_{k+1})$
...
对于任意 $R\subset\mathbb{N}^n$ 是[[MK81][递归关系]]存在[[MK139][表达式]] $\phi(x_1,...,x_{n+1})$ 使得对于[[MK27][自然数标准模型]] $\mathfrak{N}$ 及其任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$
