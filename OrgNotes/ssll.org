...
对于任意 $\phi(x_1,...,x_n)\in\mathcal{L}_{\mathcal{A}_{PA}}$ 以及任意 $\mathcal{L}_{\mathcal{A}_{PA}}$ 的任意[[~/文档/note/数理逻辑/sllj.org::MK153][结构]] $\mathcal{M} =\left<M,I\right>\vDash T_{PA}$ 

**** 无界 $\mu$ 算子<<MK26>>

对于任意 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 定义 $V=\left\{a\big|a\in M\land\mathcal{M}\vDash\phi(x_1,...,x_n)[a,v(x_2),...,v(x_n)]\right\}$ 若有 $V\ne\varnothing$ ，则 $\mu(\mathcal{M},v,(\exists x_1\phi))$ 满足：

（1） $\mu(\mathcal{M},v,(\exists x_1\phi))\in V$

（2） 任意 $v\in V$ 均有 $\left<v, \mu(\mathcal{M},v,(\exists x_1\phi))\right>\not\in I(p_<)$

***** 性质<<MK28>>

$\mu$ 是[[~/文档/note/离散数学/lssx.org::MK175][函数]]
...
即对于给定 $\mathcal{M},v,(\exists x_1\phi)$ 有 $\mu(\mathcal{M},v,(\exists x_1\phi))$ 有且仅有一个值

**** 有界 $\mu$ 算子

对于任意 $b\in M$ 以及任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 满足 $v(x_{n+1})=b$ 若有 $\left<\mathcal{M},v\right>\vDash(\exists x_1\phi(x_1,...,x_n))$

定义 $V=\left\{a\big|a\in M\land\mathcal{M}\vDash(\phi(x_1,...,x_n)\lor(x_1\hat{=}x_{n+1}))[a,v(x_1),...,v(x_n),b]\right\}$ 若有 $V\ne\varnothing$

则 $\mu_{x_1\leq b}(\mathcal{M},v,(\exists x_1\phi))$ 满足：
...
对于任意 $\mathcal{L}_{\mathcal{A}_{PA}}$ 的任意[[~/文档/note/数理逻辑/sllj.org::MK153][结构]] $\mathcal{M} =\left<M,I\right>\vDash T_{PA}$

**** 非负减

$\text{Sub}(a,b)$ 等价与 $\mu(\mathcal{M},v,(\exists x_1((f_+(x_3,x_1)\hat{=}x_2)\lor p_<(x_2,x_3))))$ 其中：
...
若 $\left<a,b\right>\in I(p_<)$ 则有 $\text{Sub}(a,b)=I(c_0)$

****** 证明

定义 $\phi(x_1,x_2,x_3)=((f_+(x_3,x_1)\hat{=}x_2)\lor p_<(x_2,x_3))$ 若 $\left<a,b\right>\in I(p_<)$ 则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]可得对于任意 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\mu$
...
即可得 $\left<a,b\right>\in I(p_<)$ 则有 $\text{Sub}(a,b)=\mu(\mathcal{M},v,(\exists x_1\phi))=I(c_0)$1

**** 整除

$\text{Div}(a,b)$ 等价于 $\mu\left(\mathcal{M},v,(\exists x_1(x_2\hat{=}f_\times(x_1,x_3))\right)$ 其中：
...
$\text{OP}(a,b)$ 等价于 $\mu(\mathcal{M},v,(\exists x_1(x_1\hat{=}f_+(f_+(f_\times(f_+(x_2,x_3),f_+(x_2,x_3)),x_2)),\tau_1))$ 其中
...
若 $\mathcal{M}=\mathcal{N}$ 是[[MK27][自然数模型]]，则 $\text{OP}(a,b)$ 是 $\mathbb{N}\times\mathbb{N}\to\mathbb{N}$ 的[[~/文档/note/离散数学/lssx.org::MK176][单射函数]]

****** 证明

对于任意 $\text{OP}(a_1,b_1)=\text{OP}(a_2,b_2)$ 有 $(a_1+b_1)^2+a_1+1=(a_2+b_2)^2+a_2+1$ 即有 $s_1^2+a_1=s_2^2+a_2$

其中 $s_1\geq a_1,s_2\geq a_2$ 。假设 $s_1\ne s_2$ 不妨设 $s_1>s_2$ 则有 $s_1-s_2\geq a_1-a_2$

由于均为自然数，且 $s_1^2-s_2^2>0$ ，故可得 $s_1^2-s_2^2\geq a_2^2-a_1^2=(a_2+a_1)(a_2-a_1)$

若 $a_1+a_2=0$ ，则根据 $s_1^2+a_1=s_2^2+a_2$ 以及均为自然数有 $s_1^2=s_2^2$ 即有 $s_1=s_2$ 与假设矛盾。故 $a_1+a_2>0$

若 $a_1+a_2=1$ ，由于均为自然数，必有 $a_1=0,a_2=1$ 或者 $a_1=1,a_2=0$ ，不妨设前者

则有 $s_1^2=s_2^2+1$ 由于均为自然数，则必有 $s_1^2=1,s_2^2=0$ 进而 $1+b_2=a_2+b_2=0$ 显然 $b_2$ 不为自然数，矛盾。

同理可退出当 $a_1=1,a_2=0$ 时亦有矛盾。即有 $a_1+a_2\ne1$ 。结合 $a_1+a_2>0$ 可得 $a_1+a_2>1$

则有 $(a_2+a_1)(a_2-a_1)> a_2-a_1$ 即有 $s_1^2-s_2^2>a_2-a_1$ 即 $s_1^2+a_1>s_2^2+a_2$ 显然与 $s_1^2+a_1=s_2^2+a_2$ 矛盾

同理可证若 $s_2>s_1$ 亦矛盾。故综上所述，假设不成立，即若 $\text{OP}(a_1,b_1)=\text{OP}(a_2,b_2)$ 必有 $s_1=s_2$

进一步有 $a_1=a_2$ ，又根据 $s_1=a_1+b_1,s_2=a_2+b_2$ 有 $b_1=b_2$

**** $\beta$ 函数

$\beta(a,i)$ 等价于 $\mu_{x_1<\text{Sub}\left(a,I(f_s)(I(c_0))\right)}\left(\mathcal{M},v,()\right)$
