...
对于任意 $b\in M$ 以及任意[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 定义 $V=\left\{a\big|a\in M\land\mathcal{M}\vDash(\phi(x_1,...,x_n)\lor(x_1\hat{=}x_{n+1}))[a,v(x_1),...,v(x_n),b]\right\}$
...
根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $\left<\eta'(x_1),\bar{\eta'}(\tau)\right>\in I(p_<)$ 由于 $\tau$ 中不含 $x_1$ 故根据[[~/文档/note/数理逻辑/sllj.org::MK155][局部确定]]易证 $\bar{\eta'}(\tau)=e(\bar{v}(\tau))$ 显然 $\bar{v}(e)=k\in\mathbb{N}$
...
$\text{OP}(\mathcal{M},a,b)$ 等价于 $\mu_{x_1}(\mathcal{M},v,(x_1\hat{=}op(\tau_a,\tau_b)))$ 其中

（1） $\mu$ 是[[MK26][无界算子]]

（2） $v$ 是 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]]满足 $\bar{v}(\tau_a)=a\in M,\bar{v}(\tau_b)=b\in M$

（3） $\tau_1$ 遵循[[MK13][ $\tau_n$ 定义]]

***** 等价[[~/文档/note/数理逻辑/sllj.org::MK129][表达式]]<<MK54>>

定义 $op(x_1,\tau_a,\tau_b)=\mu(\left(\tau_a\hat{=}f_+(f_+(f_\times(f_+(\tau_a,\tau_b),f_+(\tau_a,\tau_b)),\tau_a),\tau_1)\right),x_1)$ 其中 $\mu(\phi,x_1)$ 是[[MK26][无界算子]]的[[MK52][等价表达式]]
...
可得 $\text{OP}(\mathcal{M},a,b)=v(x_1)$ 的充要条件是 $\left<\mathcal{M},v\right>\vDash op(x_1,\tau_a,\tau_b)$
...
$\beta(\mathcal{M},a,i)$ 等价于 $\mu_{x_1<\text{Sub}\left(\mathcal{M},a,I(f_s)(I(c_0))\right)}\left(\mathcal{M},v,\left(\exists x_{4<\tau_a}\left(\exists x_{5<\tau_a}\left((\tau_a\hat{=}op(x_4,x_5))\land\phi(x_4,x_1,\tau_i,x_5)\right)\right)\right)\right)$

其中：

（1） $\{a,i\}\subset\mathbb{N}$

（2） $\text{Sub}\left(\mathcal{M},a,I(f_s)(I(c_0))\right)$ 为[[MK30][非负减]]

（3） $\mu_{x_1<\text{Sub}\left(a,I(f_s)(I(c_0))\right)}$ 为[[MK29][有界算子]]
...
（1） $sub(x_8,a,\tau_1)$ 为[[MK30][非负减]]的[[MK54][等价表达式]]
...
若有 $\left<\mathcal{N},v\right>\vDash\left(\exists x_{4<\tau_a}\left(\exists x_{5<\tau_a}\left((\tau_i\hat{=}op(x_4,x_5))\land\phi(x_4,x_1,\tau_i,x_5)\right)\right)\right)$



********* 必要

若有 $\left<\mathcal{M},v\cdot e\right>\vDash\left(\exists x_{4<\tau_a}\left(\exists x_{5<\tau_a}\left((\tau_i\hat{=}op(x_4,x_5))\land\phi(x_4,x_1,\tau_i,x_5)\right)\right)\right)$
...
又根据[[MK28][性质]]可得 $\mu_{x_1<\text{Sub}\left(a,I(f_s)(I(c_0))\right)}\left(\mathcal{M},v,\left(\exists x_{4<x_2}\left(\exists x_{5<x_2}\left((x_2\hat{=}op(x_4,x_5))\land\phi(x_4,x_1,x_3,x_5)\right)\right)\right)\right)$ 是[[~/文档/note/离散数学/lssx.org::MK175][函数]]
...
则有 $\left<\beta(\mathcal{M},a,i) ,\text{Sub}\left(\mathcal{M},a,\bar{I}(\tau_1)\right)\right>\in I(p_<)$ 或 $\beta(\mathcal{M},a,i)=\text{Sub}\left(\mathcal{M},a,\bar{I}(\tau_1)\right)$
...
（2） 对于任意 $i\in\mathbb{N}\land1\leq i\leq n$ 均有 $a_i<a$
...
易证 $op^*(a,b)=\mu_{x_1}(\mathcal{N},v,(\exists x_1(x_1\hat{=}op(x_2,x_3))))$ 其中 $v(x_2)=a,v(x_3)=b$
...
又由于 $a=op(y,z)$ 故对于任意 $i\in\mathbb{N}\land1\leq i\leq n$ 均有 $a>y>a_i$
...
（3） 对于任意 $i\in\mathbb{N}\land1\leq i\leq n$ 均有 $a_i<a$ 且 $n<a$
...
对于任意 $\mathcal{L}_{\mathcal{A}_N}$ 的[[~/文档/note/数理逻辑/sllj.org::MK153][结构]] $\mathcal{M}=\left<M,I\right>$ 以及任意 $a\in\mathbb{N}$ 有 $a\in\text{Seq}$ 的充要条件

是 $\mathcal{M}\vDash\left(\left(\exists x_1\left(b(x_1,\tau_a,c_0)\land\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)\right)\right)\lor(\tau_a\hat{=}c_0)\right)$
...
根据[[MK60][引理4]]可得充要条件为 $\mathcal{M}\vDash(\tau_a\hat{ =}c_0)$

即成立若 $a\ne0$ 且 $a\in\text{Seq}$ 则根据[[MK38][定义]]存在 $\left<a_1,...,a_n\right>\in\mathbb{N}^n$ 满足 $a=\prec a_1,...,a_n\succ$

根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]以及[[MK56][等价表达]]可得其充要条件是存在 $\mathcal{N}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $v$ 满足 $v(x_1)=n$ 以及 $v(x_{i+i})=a_i$

使得 $\left<\mathcal{N},v\right>\vDash b(x_1,\tau_a,c_0),\left<\mathcal{N},v\right>\vDash b(x_{1+i},\tau_a,\tau_i),i\leq n$

******* 充分

******** $\left<\mathcal{M},e\cdot v\right>\vDash b(x_1,\tau_a,c_0)$ 以及 $\left<\mathcal{M},e\cdot v\right>\vDash b(x_{1+i},\tau_a,\tau_i),i\leq n$ 

根据[[MK64][推论]]可得 $\left<\mathcal{M},e\cdot v\right>\vDash b(x_1,\tau_a,c_0),\left<\mathcal{M},e\cdot v\right>\vDash b(x_{1+i},\tau_a,\tau_i),i\leq n$

******** 定义 $\mu$

根据 $\left<\mathcal{M},e\cdot v\right>\vDash b(x_1,\tau_a,c_0)$ 以及[[MK56][等价表达]]可得 $\beta(\mathcal{M},\bar{I}(\tau_a),I(c_0))=e\cdot v(x_1)=\mu(x_1)$

根据 $\bar{I}(\tau_a)\ne I(c_0)$ 以及[[MK69][性质3]]可得 $\left<\mu(x_1),\bar{I}(\tau_a)\right>\in I(p_<)$ 根据[[MK58][前段定理]]可得必有 $\mu(x_1)\in Z=\left\{\bar{I}(\tau_k)\big|k\in\mathbb{N}\right\}$

即存在 $n_1\in\mathbb{N}$ 满足 $\mu(x_1)=\bar{I}(\tau_{n_1})$ ，又根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]以及[[MK60][引理4]]易证 $\mathcal{N}\vDash p_<(\tau_{n_1},\tau_a)$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $n_1<a$

若有 $\left<\mathcal{M},\mu\right>\vDash(p_<(x_2,x_1)\lor(x_2\hat{=}x_1))$ ，则根据[[MK60][引理4]]以及[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]类似易证存在 $n_2\in\mathbb{N}$ 满足 $\mu(x_2) =\bar{I}(\tau_{n_2})$ 且 $n_2\leq n_1$

总上所述，对于任意 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\mu$ 满足 $\mu(x_i)=e\cdot v(x_i),i\ne 2$ 均有：

（1） $\left<\mathcal{M},\mu\right>\not\vDash (p_<(x_2,x_1)\lor(x_2\hat{=}x_1))$

（2） 存在 $n_2\in\mathbb{N}$ 满足 $\mu(x_2) =\bar{I}(\tau_{n_2})$ 且 $n_2\leq n_1$

******** 定义 $\eta$

则定义 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\eta$ 满足 $\eta(x_i)=\mu(x_i),i\ne 3$ 以及 $\eta(x_3)=e\cdot v(x_{1+n_2})$

根据 $\eta(x_2)=\mu(x_2)=\bar{I}(\tau_{n_2}),n_2<n_1$ 可得 $\left<\mathcal{M},e\cdot v\right>\vDash b(x_{1+n_2},\tau_a,\tau_{n_2})$

又根据 $\eta(x_3)=e\cdot v(x_{1+n_2}),\eta(x_2)=\bar{I}(\tau_{n_2})=\overline{e\cdot v}(\tau_{n_2})$ 以及[[~/文档/note/数理逻辑/sllj.org::MK176][替换定理]]可得 $\left<\mathcal{M},\eta\right>\vDash b(x_3,\tau_a,x_2)$

******** 结论

根据[[MK68][定义]]可得 $\left<\mathcal{M},e\cdot v\right>\vDash\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)\right)$ 等价于 $\left<\mathcal{M},e\cdot v\right>\vDash\left(\forall x_2(((p_<(x_2,x_1)\lor(x_2\hat{=}x_1))\to(\exists x_3(b(x_3,\tau_a,x_2)))\right)$

即任意 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\mu$ 满足 $\mu(x_i)=e\cdot v(x_i),i\ne 2$ 均有 

（1） $\left<\mathcal{M},\mu\right>\not\vDash (p_<(x_2,x_1)\lor(x_2\hat{=}x_1))$

（2） $\left<\mathcal{M},\mu\right>\vDash(p_<(x_2,x_1)\lor(x_2\hat{=}x_1))$ 且 $\left<\mathcal{M},\mu\right>\vDash(\exists x_3(b(x_3,\tau_a,x_2)))$

     即存在 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\eta$ 满足 $\eta(x_i)=\mu(x_i),i\ne 3$ 以及 $\eta(x_3)=e\cdot v(x_{1+n_2})$ 使得 $\left<\mathcal{M},\eta\right>\vDash b(x_3,\tau_a,x_2)$

综上所述，根据前两部证明可得 $\left<\mathcal{M},e\cdot v\right>\vDash\left(b(x_1,\tau_a,c_0)\land\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)\right)$

即根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $\mathcal{M}\vDash\left(\exists x_1\left(b(x_1,\tau_a,c_0)\land\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)\right)\right)$

******* 必要

******** $\mathcal{N}\vDash b(\tau_n,\tau_a,c_0)$

若有 $\mathcal{M}\vDash\left(\exists x_1\left(b(x_1,\tau_a,c_0)\land\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)\right)\right)$ 则存在 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\mu$

满足 $\left<\mathcal{M},\mu\right>\vDash\left(b(x_1,\tau_a,c_0)\land\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)\right)$ 根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]

（1） $\left<\mathcal{M},\mu\right>\vDash b(x_1,\tau_a,c_0)$

（2） $\left<\mathcal{M},\mu\right>\vDash\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)$ 

根据[[MK56][等价表达]]可得 $\beta(\mathcal{M},\bar{I}(\tau_a),I(c_0))=\mu(x_1)$ ，根据 $\bar{I}(\tau_a)\ne I(c_0)$ 以及[[MK69][性质3]]可得 $\left<\mu(x_1),\bar{I}(\tau_a)\right>\in I(p_<)$

由于 $\tau_a\in Z=\left\{\bar{I}(\tau_k)\big|k\in\mathbb{N}\right\}$ 故根据[[MK58][前段定理]]可得必有 $\mu(x_1)\in Z$ ，即存在 $n\in \mathbb{N}$ 满足 $\mu(x_1)=\bar{I}(\tau_{n})$

根据[[~/文档/note/数理逻辑/sllj.org::MK176][替换定理]] $\mathcal{M}\vDash b(\tau_n,\tau_a,c_0)$ 则根据[[MK64][推论]]可得 $\mathcal{N}\vDash b(\tau_n,\tau_a,c_0)$

******** 对于任意 $i\leq n$ 有 $a_i\in\mathbb{N}$ 使得 $\mathcal{N}\vDash b(\tau_{a_i},\tau_a,\tau_n)$

显然有 $\mathcal{N}\vDash(p_<(\tau_i,\tau_n)\lor(\tau_i\hat{=}\tau_n))$ 根据[[MK60][引理4]] $\mathcal{M}\vDash(p_<(\tau_i,\tau_n)\lor(\tau_i\hat{ =}\tau_n))$

则定义 $\mathcal{M}$ 的[[~/文档/note/数理逻辑/sllj.org::MK154][赋值]] $\eta$ 满足 $\eta(x_1)=\bar{I}(\tau_n)=\mu(x_1)$ 且 $\eta(x_2)=\bar{I}\tau_n$ 根据[[~/文档/note/数理逻辑/sllj.org::MK176][替换定理]] $\left<\mathcal{M},\eta\right>\vDash(p_<(x_2,x_1)\lor(x_2\hat{ =}x_1))$

显然 $\eta$ 亦满足 $\eta(x_i)=\mu(x_i),i\ne 2$ 。根据[[MK68][定义]]可得 $\left<\mathcal{M},\mu\right>\vDash\left(\forall x_2_{\leq x_1}(\exists x_3(b(x_3,\tau_a,x_2)))\right)\right)$ 

等价于 $\left<\mathcal{M},\mu\right>\vDash\left(\forall x_2(((p_<(x_2,x_1)\lor(x_2\hat{=}x_1))\to(\exists x_3(b(x_3,\tau_a,x_2)))\right)$

则根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]] $\left<\mathcal{M},\eta\right>\vDash((p_<(x_2,x_1)\lor(x_2\hat{=}x_1))\to(\exists x_3(b(x_3,\tau_a,x_2))))$

又由于 $\left<\mathcal{M},\eta\right>\vDash(p_<(x_2,x_1)\lor(x_2\hat{ =}x_1))$ 故根据[[~/文档/note/数理逻辑/sllj.org::MK158][满足]]必有 $\left<\mathcal{M},\eta\right>\vDash(\exists x_3(b(x_3,\tau_a,x_2)))$

即存在 $\eta'$ 满足 $\eta'(x_i)=\eta(x_i),i\ne 3$ 使得 $\left<\mathcal{M},\eta'\right>\vDash b(x_3,\tau_a,x_2)$ 根据[[MK56][等价表达]]可得 $\beta(\mathcal{M},\bar{I}(\tau_a),\eta'(x_2))=\eta'(x_3)$

根据[[MK69][性质3]]可得 $\left<\eta'(x_3),\bar{I}(\tau_a)\right>\in I(p_<)$ ，根据[[MK58][前段定理]]可得必有 $\eta'(x_3)\in Z$ 即有 $a_i\in\mathbb{N}$ 满足 $\eta'(x_3)=\bar{I}(\tau_{a_i})$

则根据[[~/文档/note/数理逻辑/sllj.org::MK176][替换定理]] $\mathcal{M}\vDash b(\tau_{a_i},\tau_a,\tau_n)$

综上所述，对于任意 $i\leq n$ 均有 $a_i\in\mathbb{N}$ 满足 $\mathcal{M}\vDash b(\tau_{a_i},\tau_a,\tau_n)$ 则根据[[MK64][推论]]可得 $\mathcal{N}\vDash b(\tau_{a_i},\tau_a,\tau_n)$

******** 结论

根据上上步有 $\mathcal{N}\vDash b(\tau_n,\tau_a,c_0)$ 即根据[[MK56][等价表达]]可得 $\beta(\mathcal{N},a,0)=n$

根据上一步有对于任意 $i\leq n$ 有 $a_i\in\mathbb{N}$ 使得 $\mathcal{N}\vDash b(\tau_{a_i},\tau_a,\tau_n)$ 同理 $\beta(\mathcal{N},a,i)=a_i$

即根据[[MK38][定义]]可得 $a=\prec a_1,...,a_n\succ$ ，故有 $a\in\text{Seq}$ 
