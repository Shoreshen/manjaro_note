...
对于任意 $a\in\mathbb{N}$ 若 $a=\prec\lceil x_i\rceil\succ$ 即 $a=\prec J(7,i)\succ$ 则 $a$ 是[[~/文档/note/数理逻辑/sllj.org::MK107][变元符号]]的哥德尔数。则 $\text{Vrble}(a)$ 为真

$(b(\tau_1,\tau_a,c_0)\land b(x_1,\tau_a,\tau_1)\land(\exists x_2\text{J}(x_1,\tau_7,x_2)))$
...
对于任意 $R\subset\mathbb{N}^m,Q\subset\mathbb{N}^n$ 对于任意 $z\in\mathbb{N}$ 任意[[~/文档/note/离散数学/lssx.org::MK175][函数]] $p:\{1,..m+n\}\to\{1,...,z\}$ 以及任意 $\vec{a}\in\mathbb{N}^n$ 定义如下：
...
（4） $(\vec{a}\in R\leftrightarrow_p Q)$ 等价 $(\vec{a}\in R\to_p Q)$ 且 $(\vec{a}\in Q\to_p R)$ 

以上三种均为关系在 $p$ 下的布尔组合，并称 $p$ 为 $\left<a_{p(1)},...,a_{p(m)},a_{p(m+1)},...,a_{p(m+n)}\right>$ 到 $\vec{a}$ 的投影。

特别的，当 $z=n$ 且 $p(i)=\begin{cases}i&i\leq n\\i-n&else\end{cases}$ 时省略 $p$ 记作 $R\land Q,R\lor Q,R\to Q,R\leftrightarrow Q$ 

另对于任意 $a\in\mathbb{N}^m$ ：
...
根据[[MK30][定义]]以及[[MK66][性质1]]可得 $\text{Sub}(\mathfrak{N},a,b)=\begin{cases}b-a&a<b\\0&else\end{cases}$ 根据[[MK90][性质9]] $f(a,b)=\mu_x(\chi_{((b+x=a)\lor_p(a<b))}(a,b,x)=1)$ 是[[MK81][递归函数]]
