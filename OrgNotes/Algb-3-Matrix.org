...
若 $k\in K$ 其中 $K$ 为[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK24][数域]]， $A,B$ 为可相乘的矩阵，则有 $(kA)B=k(AB)=A(kB)$
...
显然 $\left(I\lambda-A\right)\vec{x}=\vec{0}$ 可对应[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK28][其次线性方程组]]，由于 $\vec{x}$ 为非零向量，即有非零解

则根据[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK30][推论]]可得 $|I\lambda-A|=0$ 进而求解使行列式为零的 $\lambda$

** 特征多项式<<MK67>>

定义 $\lambda$ 为变量，根据 $|I\lambda-A|=0$ 又根据[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK11][行列式定义]]可得 $|I\lambda-A|$ 是关于 $\lambda$ 的[[~/OneDrive/高等代数/Algb-4-Multi-equ.org::MK1][一元多项式]]

故可得特征值为 $|I\lambda-A|$ 在[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK24][域]] $F$ 上的所有[[~/OneDrive/高等代数/Algb-4-Multi-equ.org::MK52][根]]
...
根据[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK32][行列式与乘法]]可得 $|I\lambda-A|=|P^{-1}||\lambda I- B||P|=|P^{-1}||P||\lambda I- B|$

根据[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK33][可逆矩阵与行列式]]可得 $|I\lambda-A|=|\lambda I- B|$ 根据 $\lambda$ 的任意性可得[[MK67][特征多项式]]相等
...
其中 $\overline{M}^*\left(\begin{matrix}j_{n,i,1}&j_{n,i,2}&...&j_{n,i,k}\\j_{n,i,1}&j_{n,i,2}&...&j_{n,i,k}\end{matrix}\right)$ 是 $A$ 去除 $j_{n,i,1},...,j_{n,i,k}$ 行 $j_{n,i,1},...,j_{n,i,k}$ 列的[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK22][余子式]]

**** 证明

显然当 $k=0$ 时 $C_n^0=1$ 则 $\sum_{i=1}^{C_n^0}\left|Z_n(\{j_{n,i,1},...,j_{n,i,k}\})\right|=\left|Z_n(\varnothing)\right|$ 即 $Z_n(\varnothing)=[-a_{ij}]$

则根据[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK14][行乘系数]]可得 $\sum_{i=1}^{C_n^0}\left|Z_n(\{j_{n,i,1},...,j_{n,i,k}\})\right|=(-1)^n|A|$
...
则根据[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK18][上三角矩阵行列式]]可得 $\sum_{i=1}^{C_n^n}\left|Z_n(\{j_{n,i,1},...,j_{n,i,k}\})\right|=\lambda^n$
...
分别为 $Z_n(\{j_{n,i,1},...,j_{n,i,k}\})$ 对于 $j_{n,i,1},...,j_{n,i,k}$ 行 $i_1,...,i_k$ 列的[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK21][子式]]和[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK22][余子式]]
...
故和中非零[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK21][子式]]有且仅有 $M\left(\begin{matrix}j_{n,i,1}&j_{n,i,2}&...&j_{n,i,k}\\j_{n,i,1}&j_{n,i,2}&...&j_{n,i,k}\end{matrix}\right)$ 且为对角矩阵的行列式，对角线上的值均为 $\lambda$
...
可得 $\overline{M}\left(\begin{matrix}j_{n,i,1}&j_{n,i,2}&...&j_{n,i,k}\\j_{n,i,1}&j_{n,i,2}&...&j_{n,i,k}\end{matrix}\right)=|[m_{ij}]|$ 有 $m_{ij}=-a_{ij}$ 故根据[[~/OneDrive/高等代数/Algb-1-Liner_sys_func.org::MK14][行乘系数]]可得
...
定义矩阵 $P=[x_{ij}]$ ，由于线性无关，故根据[[~/OneDrive/高等代数/Algb-2-Liner_Space.org::MK57][行列式与线性相关]]可得 $|P|\ne0$ 进一步根据[[MK29][可逆充要条件]]可得存在逆矩阵 $P^{-1}$
